...
npm run verify:core       # 113 pure-Node/tsx logic test suites (incl. verify-serve: real bundle child process;verify-build-manifest 无 dist 时 SKIP,CI 在 vite build 后另跑)
...
- **Tests live in `scripts/verify-*.mjs`** (113 suites) — run via `tsx`, import real TS source.
