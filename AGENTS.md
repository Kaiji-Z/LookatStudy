...
| Reasoning effort | `shared/reasoning-effort.ts` | 思考强度方言表(v0.10,存 `settings.reasoning_effort`:""自动/fast/deep):GLM→body.thinking.type、Qwen/SiliconCloud→enable_thinking(经 `llm-client.buildLanguageModel` 的 fetch 覆盖注入)、OpenAI→reasoningEffort、Anthropic/Google→providerOptions;支持的家族:GLM/DeepSeek/Kimi/豆包/StepFun(布尔/对象/字符串参数按各家方言)/xAI(按模型门控,裸 grok-4 传参报错自动落 none);不支持的家族(MiniMax 官方无开关、百度预算式形状特殊)芯片禁用+引擎降级 none,宁可不生效不瞎发参数。**家族嗅探必须传 hints**:UI 门控(supportsReasoningControl)与引擎调用点(reasoningPlanFor)都按 baseUrl/模型名嗅探 glm-codingplan 预设与 custom-*(2026-08-18 修:引擎调用点曾漏传 hints,fast 对它们静默失效——开了快速仍思考 9187 字);verify T6d 是源码级接线守卫 |
