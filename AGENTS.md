...
- **伴学伙伴 v2 (companion)**: 五形态 cel 风格 SVG 守护机——小焰 ember(珊瑚+青屏)/霜绒 frost(冰晶棱面)/苔芽 moss(双子叶生长)/星尘 astro(暗夜天文台)/墨墨 ink(宣纸朱砂),设置 `companion_form` 切换,设置页选择器带实时 Mascot 预览(点选即换,广播 `companion-config-changed`)。架构=**壳+皮肤**:`Mascot.tsx` 只做生命系统壳(拍臂 WAAPI/眨眼/视线跟随+漫游/麦克风声波弧 rAF),`forms/registry.tsx` 聚合五份皮肤,共享 FormRefs 套件(bot/head/armL/armR/eyes/pupils/waves)与类契约(cp-bot/cp-arm/cp-antenna/…),姿态 CSS、按压动画、眨眼、视线对全部形态零改动生效;`forms-index.ts` 纯素 id/默认值/脏值回退(verify T12 守注册表咬合)。**三栖息地**:左栏地图天空守望(`RailCompanion`)+右栏笔记本导师(`NotebookCompanion`,选中节点时)+中栏对话区右上(`ChatCompanion`,T3 单栏对话视图也常驻)。**Bongo Cat 式输入反馈**(`bus.ts` 全局监听,渲染层零 IPC):逐键交替拍臂(keydown→press 事件→WAAPI ±26° 侧向击打,typing 姿势 1.2s 超时回落)、点击侧向拍臂(按屏幕左右半屏定方向)、真实麦克风声波弧(听写 RMS→smoothMic attack 0.55/release 0.12→4 档弧缩放,rAF 直写 transform 零重渲染)、发送消息出拳(happy+punch 700ms)、窗口失焦 4s 快速打盹/聚焦回来跳一下问好(幂等)。全动作反馈经 `lib/companion/bus.ts` 全局店(celebration 总线+`state:changed`+talking/listening/streaming 旗标);朗读口型同步六档 viseme 同 v1(`use-mouth.ts`+共享 AnalyserNode);各形态自有能量核(XP 充能:火苗充填/六瓣冰晶/双子叶开角/月相位移/墨池液位)+ streak 天线签名;reduced-motion 退静态;点击只落涂色区不挡地图球/选区。
