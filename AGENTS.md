...
- **Renderer 非首屏组件一律 React.lazy (v0.22 入口包瘦身)**: SettingsView/ExamView/CommandPalette/CourseSearchPanel/artifact 渲染器(artifacts/index)/CanvasStage/SelfRatingCard/VoicePanel 按需拉 chunk(打开时才加载,主束 main-*.js gzip 397→276KB)。新增重组件先问"首屏要不要"——伴学伙伴/i18n/基础 markdown 管线例外(常驻)。
...
- **Left (MapRail)**: Duolingo-style skill map. 顶部课程名可点(2026-09-12):弹出全部课程列表浮层,点选切课(`map-course-switch`/`map-course-menu`),与导入页列表同源 `onSelectCourse`. A node is a *session group*: clicking it filters the middle pane's threads by `focus_node_id`. Node states: locked / available / in_progress / mastered. Collapsible (`Ctrl+B`).
- **伴学伙伴 v3 (companion, 单生物连续体)**: **全应用唯一一只**(`CompanionCreature` 根层 fixed 覆盖层,App 挂一次,`worldReady`=选中课程),三重世界连续行动(zone 状态机:优先级 输入框聚焦>朗读/记笔记>回左栏老家;回家等 `ZONE_RETURN_MS` 3.5s 防抖,直去中/右栏即时)。**左栏=原生物理世界**:`companion-flight.ts` 独立 Matter 引擎(视口空间,不进岛引擎——岛有顶墙会把生物关进一个 section)+PD 悬浮控制器(重力补偿/加速度钳制/确定性利萨茹巡航)——像钢铁侠悬浮;**自主巡航避让**(`avoidAccel` 感知域推离+速度感知制动,近距避让>巡航弹簧——宁可绕路不穿球,仿真断言最小间距>接触距,球布局不被自主行动搅乱;晕眩期不避让,用户拖球拍他不受影响);与地图球**跨引擎手工碰撞**(`crossImpulse`:重叠→法线/冲量按质量比分摊,球重伴学轻→球拍他=翻滚晕眩 900ms+surprised,他撞球=球荡开经 BallProbe.push 回灌岛引擎;锁定球=不可撼动);MapRail 经事件注册岛+nav+可见性进 `RailWorld`(bus ref,rAF 直读零重渲染)。**中栏=宠物世界**:textarea onFocus→飞来悬在输入卡上空(v5 放大+漂浮,见下),Bongo Cat 逐键拍臂(v5 加强);落栖有一次 WAAPI 压缩回弹(composite add,不碰 rAF 位移)。**动作细化**:poke 花样轮换(`pokeSeq` hop/wave/spin 循环)、窗口聚焦回归=挥手打招呼、口型宽度调制(`MOUTH_WIDTH`:E/I 横宽 O/U 收窄,口型高×宽两维)、星星眼闪烁(cp-star-tw)、睡眠呼吸(cp-sleep-breath)。**右栏=助教世界**:朗读(talking,信号**下沉到 useSpeech 引擎层**按 speakingMessageId 发,旧组件层对比法曾静默失效)→口型同步+栖右上(朗读跟句时被 .cp-reading-mark 实时位置接管,见 v6);划线加笔记(`companionNote`)→writing 姿势钉住 2.2s。**纱帘待机**:无交互 9s 或滚动中 2.5s→veil(半透明+轻模糊),点击/任意活动唤醒回前台;**待机/巡航锚点=`pickPerchBase`**:高度中部带 5×3 候选网格选离所有球最远的空地,每 3s/栏宽变化重选(纱帘后也照常选点,只是不喂物理)。**跨栏 transit**:zone 切换→cp-pose-flying(双臂后掠)+跟随倾角,950ms 姿势窗;目标栏不可见(T3 换栏)自动退回 rail, rail 也不可见则隐匿。五形态/壳+皮肤/设置选择器同 v2(`companion_form`)。**组件→bus 触发一律走 window CustomEvent**(`fire()`广播+install() 监听,与 companion-config-changed 同款——防打包器把 bus 解析成双实例直调丢事件;verify T15 守源级接线)。reduced-motion 物理与跟随全旁路静态贴锚;点击只落涂色区(svg visiblePainted)不挡底下 UI。
  - **v13 标题栏栖息地+流式思考召唤(2026-08-24,用户反馈加笔记后多余停留/笔记tab误召唤)**:①左栏不在场的家=v12 左缘停靠退役→**标题栏栖息**(`titlebarRoam` 纯函数,SIZE.titlebar=44,沿 header.app-header 横向利萨茹缓游 ~40s/来回,y 钉栏中心);与栏内可交互元素(button/[role=button]/a/input/select,150ms 矩形缓存)重叠→`cp-titlebar-dim` 半透明+svg pointer-events 穿透;禁入带钳制豁免 titlebar(家就在带里);左缘停靠只留跟句悬空兜底一处。②**拖拽语义**:松手在标题栏内=栖栏;栏外=手放落点(`manualHomeRef`,onUp 判 railOk false 时记录,`manualHomeWander` 原地 ±22/16 徘徊,pane 按落点 nb/chat),railOk 上升沿清空(有栏可归时家=左栏)。③**流式思考召唤**:desiredZone chatActive 加 `‖streaming`(App 既有 companionSetStreaming 接线),流式中飞输入卡上空 thinking 姿势,结束经防抖回家。④**笔记直回**:nb 定时信号(zoneNbUntil≠0)到期跳过 ZONE_RETURN_MS 防抖立即回家(写作 2.2s 即驻留,不再多呆右上肩 3.5s);talking(非定时)仍走防抖;笔记 tab 点击不再 companionNote(切标签不是记笔记)。verify T13 行为断言+T15 十项守卫;ui-test v9=空态标题栏带内/v10=轮询容忍瞬态/T3=rail 卸载后 zone=titlebar。
  - **v0.28 视觉审查打磨(2026-09-02,实机 serve+Playwright 逐状态截图审查驱动)**:①**朗读避让整句化(压句 bug 根修)**——避让障碍从"已播前缀 Range 行盒"升级为**整条显示句**行盒:v11.4 起 mark 只覆盖已播前缀,长句被强断成多块时只让开前缀=身体压住同一句已高亮未读行(实测 6s 稳定压句 ~2000px² 且 occluding=false——对片段合法对整句违例)。`shared/speech-text.ts` 新增 `displayGroupSpanAround`(块段→句组跨度,句界判定与 splitSentences 同源:终点块含闭合符不外扩/向前扫到文本头即句头/maxSpan 防病态长文),highlightText 双 Range 登记(`readingSentenceRange`,前缀 Range 仍只管高亮),Creature 避让消费 `getReadingSentenceRange() ?? readRange`(T15 守消费点级接线,防 includes 弱标记);writing 落点同理避让画线所在段落逐行行盒(块级 Element.getClientRects 只回巨盒,必须 Range)。②**星尘亮缘**:astro 外轮廓描边 OUT→RIM(#A79BFF,7 处主轮廓/耳舱/天线环/尾焰锥)——旧深紫描边与左栏夜空几乎同色整体隐身;家族"暗描边"语法对暗色身体的反转延伸,内部细节仍 OUT。③**纱帘分档**:`[data-zone="rail"].cp-veil` 0.3→0.55+blur(0.5px)——地图锁定深色,0.3 叠夜空待机点全隐形;内容栏浅底维持原值。④**栏缘净空**:pickPerchBase 评分 `clear` 初始值从 ∞ 改为 `min(cx, w-cx)`——离缘距离与离球距离同台比较,空旷地图不再选中 0.86w 贴分界线列("挤墙角"观感)。⑤**庆祝落点外推**:celebrationPerch x 加 `+0.4×size`(origin=题卡右上角内侧点,旧 +36<身体半宽必骑题干/选项行),身体左缘恒在 origin 右侧。verify:T11b 跨度边界+T14 栏缘+T19 落点外推+T15 消费点守卫;实机量化复验=朗读稳态核心盒与高亮重叠 0px。
  - **v0.36 开屏导师空态剧本**:未选课态中栏=BootGuidePanel(bot 主持冷启动/回访建议),生物挂载即 `companionZoneFocus(true)` 召唤坐镇、卸载放手回家;锚点=chatAnchor 兜底链末位 `data-companion-anchor="boot-guide"`(无输入卡时);答题/选 MBTI 表情爆发 `companionNodePoint`,试玩题答对走 celebrate 总线(既有监听);verify T15 守三项(锚/召唤/表情)。
- **v12 召回制(2026-08-23,用户反馈遮挡阅读)**:游走只锁左栏(`nextRoamPane` 恒 rail,环形跨栏退役);中/右栏=召唤制——输入框聚焦/**听写 listening**/朗读/记笔记等 duty 信号飞来(`desiredZone` 的 chatActive=zoneChatAt‖listening),信号结束(ZONE_RETURN_MS 防抖)回家;**庆祝召唤**:答对(题卡 QuizArtifact/复习自评 ReviewPanel)与解锁(MapRail)随 celebrate 总线带 `origin` 事件坐标,伴学飞到源旁庆祝 ~3s(`celebrationPerch`)再回落;锚点健壮性=球 rect 必须真在视口内才携带(onScreen 守卫,虚拟化屏外球负坐标不带)+落点 `elementFromPoint` 无宿主不召唤(不兜底猜栏);答错/其他高光不召唤,考试中不抢陪考位。左栏不在场时的家见 v13 标题栏栖息(左缘停靠已退役);朗读无跟句锚时讲解面板右上肩停靠(wanderInPanel 只许喂 navRect);**拖拽松手同步 rail 刚体到落点**(onUp 里 Body.setPosition,否则滑回拖拽前旧坐标=观感"回原点");`CRUISE_HOME=0.5` 回家档(ROAM<HOME<OP,有来有往)。verify T19 召回制/巡航档序 + T15 守卫;**ui-test 窗口必须透明 show**(src/main/index.ts runUiTest)——从未 show 的窗口拿不到合成器 begin-frame,rAF 1fps 后全停,伴学动画断言假红(backgroundThrottling:false 救不了合成器)。
  - **v4 细化**:情境反应(App 派发——进考试=cheer 挥手后离场零干扰/复习抽屉开=自豪待命/隔天回来 `getStreak().lastActiveDate<今天`=星星眼转圈;MapRail 派发——导入 `companionImporting` 监工旗(豁免纱帘+地图面板隐去也在栏内值守)+`importDone` 成功欢呼/失败鼓励/取消无反应);**抓取拖拽扔出**(Mascot onPointerDown+window pointermove/up 采样速度,抓=挣扎+物理旁路,快扔=真实初速+throwDizzy 翻滚→回稳后 huffy 鼓脸余怒,慢放=温柔;touch-action none 触屏同款);**宠物音效**(`pet-sfx.ts` WebAudio 合成零资产,戳/开心/被拍/落地/抓住 五枚低音量啾哔,设置 `companion_sfx` 开关);**记忆联动**(Creature 按 courseId 调 `getDashboard().frictionByNode[0]`,把待机空地钉到卡点球旁 8s+nodePoint 托脾指向);**等级徽标**(bus 读 XpStatus.level:≥3 头顶小皇冠/≥7 金色光环,壳层渲染全形态共享);**飞行细节**(cp-thruster 喷焰粒子随飞行姿势点亮/地面阴影随 --cp-alt 高度缩放/天风 `weatherPhysFor` 吹斜+雨中 cp-shiver 抖水);**落绳栖息**(veil 时 `pickRestSpot` 选最近绳粒/球顶,settle 模式定住休息)。
  - **v5 栏内临场感**:体型 chat 76px/notebook 88px(rail 88 不变)。**中栏悬浮**:锚点 `composer-card` 上缘上空(完整可见,拍键手臂全程外露;v6 撤掉过一版半身藏卡 clip 方案——效果粗糙)。**栏内漂浮**:chat/notebook 锚点叠 `zoneDrift` 慢利萨茹漂移(chat ±9/±5px、notebook 更收敛 ±5/±4px——他在附近轻轻游动,不是钉死)。**打字反馈升级**:拍臂 ±26°→±52° + cubic-bezier(0.2,1.5,…) 回弹过冲 + 整机 translateY/scale 压弹同拍(SVG g 的 transform-origin 默认 viewBox 原点,须显式 `transform-box:fill-box` + `50% 62%`,否则从左上角缩放跑偏)。verify T14b 纯函数 + T15 源级守卫。
  - **v7 景深+伪3D+口型+起飞**:SIZE 近大远小 rail 60/chat 84/notebook 108(Mascot svg 尺寸 CSS transition=跨栏变大动画本体);`BevelPlate`(forms/shared,R-06 blk 倒角棱:左上光/右下暗,inverted=凹槽)应用到 ember 头盔/身体/耳舱/屏框+通气缝螺栓细节;起飞=WAAPI 蓄力弹射+cp-takeoff 喷焰增强;航灯 cp-beacons(左红右绿,飞行加速闪);口型=共振峰法(`visemeFromFormants` F1 180-900/F2 900-3200 能量质心→五元音锚点最近邻,use-mouth 双帧确认);**手机修复**:rail 不在场(railOk false)时 eff 回退 chat/notebook 锚点(不消失)。待做:Rhubarb 式离线口型时间轴(每句 AudioBuffer 起播前预分析→播放时钟查表)。
  - **v6 朗读句级跟随(karaoke)**:整课朗读时讲解区高亮正在读的句子 + 伴学跟句指向 + 🔊 悬浮。**播放序**:useSpeech 新增 `playingSentence`(缓冲起播计数 startedRef,非合成到达序——edge 预取深 2,到达序超前 1-2 句);句子表 v11.4 起高亮**直接吃合成侧权威原文**(ttsAudio.sentence 随块下发,渲染层 playedSentencePrefix 拼已播前缀,零复算——旧"从 content 复算句表+组号映射"两侧分叉是错句之源,详见 v9 bullet 末尾 v11.4)。**高亮**:`highlightText.markReadingSentence`(getTextModel 文本搜索三级兜底:精确→空白归一化→前缀,与持久画线同款;wrap 算法抽 `wrapRangeWithElement` 共用)+ `.cp-reading-mark`(accent 16% 底,box-decoration-break clone);句子滚出视野(±48px 容差)才 smooth center 跟随,不逐句乱跳;停止/切节点 clearReadingMark。**跟句指向**:Creature rAF 在 notebook zone 直查 `.cp-reading-mark` 元素(rect 随滚动自动更新,零事件通道),`readingAnchorPos`(纯函数:句右侧优先/超面板换左侧,y 钳制安全带)定锚,pose `point`/`pointr`(armL -92°/armR +92° 抬平指向+微倾;换侧翻转),跟句时叠 drift 关掉、lerp 110ms 平滑换句;**指臂方向约定**:SVG 正角=屏幕顺时针,下垂手臂 +92° 从 6 点扫向 9 点=向左外伸——point=armL +92°、pointr=armR −92°,符号写反手臂横穿身体藏到头后(实测踩过,T15 有方向正则锁)。**chat 消息朗读同款**:MessageRowV2 透传 playingSentence,消息容器(整条 msg,句可跨 text part)做 karaoke 搜索容器;Creature 跟句不限 notebook——mark.closest(notebook-panel/chat-stream) 取 clamp 面板与体型(chat 76/notebook 88),中栏朗读时伴学留在中栏指读。**v6 修乱序/错位**:①音频乱序根因=decodeAudioData **完成序**进队(短句先解完,合成快于收听时全员竞争——最后一句插进第二句后面念过,竞态 v0.13 起就在)→ pendingRef Map 按 sentenceIndex 建池+nextSeq 只按序消费;②句子匹配=matchSentenceTokens 纯函数(空白切 token 顺序匹配,token 间隔=段落边界/行内代码/表格竖线等读显差异;单调游标 WeakMap 防重复回跳;跨度 sanity 防误命中)+resetReadingCursor(readingIdx===0 归零);③chat 匹配 within 限定 part-text(思考块渲染但不读);④lookatstudy-speech-start 跨实例互停。verify T16 纯函数+T15 守卫。**🔊 sticky**:朗读按钮从标题行挪进 `sticky top-0` 悬浮行(讲解视口右上,吸顶行 pointer-events-none 只按钮可点)——滚到哪都能停。
  - **v8 入住感+渲染崩溃根治**:①**渲染失败双层修复**——(a) karaoke 高亮从 span 包裹迁到 **CSS Custom Highlight API**(`markReadingSentence` 返回 Range,`CSS.highlights.set("cp-reading",…)`+`::highlight(cp-reading)` 样式,**零 DOM 改动**;不支持的环境回退旧 span 包裹):旧方案改写 React 管理的 DOM,朗读逐句重渲染时 reconcile 撞外来节点即崩(重试重挂又好、下一句再炸——这就是"朗读时回到渲染失败"的复现链);Creature 跟句改读 module 单例 `getReadingRange()`(Range.getBoundingClientRect 随滚动自动更新)。(b) `course:getNodeContent` **读取口对原文与翻译都过 `sanitizeTranslatedMarkdown`**(此前仅 CDN 拉取路径净化;AI for beginners 类含断行链接的畸形 markdown 原文/翻译直喂 rehype-raw 即崩 ErrorBoundary——"部分课程才有"的根源;净化幂等,重试好转=旧 DOM 恰好重建成功,非真修)。②**右栏徘徊**:`wanderInPanel` 纯函数(利萨茹+倍频微扰)在讲解面板右侧空白带游弋,顶部 96px 标签/按钮安全带与底部 16px 让位,朗读时仍逐句跟读。③**景深调档** SIZE rail 76/chat 96/notebook 120;手机切栏体型由 `dispZone`(rAF 写 ref、变更才 setState)驱动,不再只认 desktop zone。④左栏=高空无落地影(`[data-zone="rail"] .cp-shadow` 隐藏)+钢铁侠推进喷焰(更长更亮+drop-shadow 辉光+cp-thruster-pulse 脉动)。⑤按需求**移除皇冠徽标**(光环保留)。verify T15 v8 标记(Size/无 crown/Highlight API/rail 无影/wanderInPanel 接线)。
  - **v9 口型双路径+常驻+整句高亮**:①**口型=离线时间轴**(渲染层 `viseme-timeline.ts` 纯 JS radix-2 FFT,每句 AudioBuffer 解码后预分析→cue 表,播放时钟 `ctx.currentTime-startedAt` 查表;`speech-analyser.ts` 增活动播放登记所,use-mouth 时间轴优先、实时 Analyser 退兜底)②**剧本口型**(主进程 `viseme-script.ts`:edge 档 `saveSubtitles` 开 WordBoundary 逐词 sidecar + pinyin-pro 声母映射→SpeechVisemeCue 随 ttsAudio 事件下发,`tts-cache` 加 {key}.cues.json 随音频缓存;无 cue 的引擎自动落渲染层 DSP)③词表=shared `SpeechViseme` 九形(closed/A/E/I/O/U/SS 齿擦/L 舌尖/FV 咬唇,main/renderer 同一真源);ember 三辅音嘴型全套手绘,其余形态 `coarseViseme()` 降级最近母音。④**伴学常驻**:App 无条件挂载(worldReady 门控移除),空态/两栏锚点全缺→`freeRoam` 整窗游走绝不隐匿,朗读中切课不消失。⑤**karaoke 整句对齐**:`matchSentenceAligned`(canonicalSpeechIndex 规范化:全角→半角/小写/滤标点空白+规范位→原文下标映射;整句 indexOf 优先→token 间隔兜底;句界扩展=句首吃开引号/句末吃标点闭引号)替换旧 token 匹配,修"高亮落在句中"。verify T16 重写+T17 时间轴+T18 剧本口型,T15 v9 标记含 startedAtCtxTime 实接线串(弱标记曾放过改名破坏)。**v10 连续性重设计**:zone 机第四态 `roam`(闲时跨栏游走,时间桶 `nextRoamPane` 调度 ~30% 相邻栏;栖 rail 仍物理世界,他栏面板游弋);**双槽信号** `zoneChatAt`/`zoneNbAt`(最新激活者胜,边朗读边输入跟最新动作;单对象信号会被 talking 结束连带清掉仍生效的 focus——实测踩过);**限速滑翔 `glideTo`**(指数趋近+远距封顶巡航,治闪现;CRUISE_OP/READ/ROAM 三档);朗读锚点 v11.5 重做 `readingAnchorFlex`(用户拍板:生物不压**正在高亮的句**,压未高亮文字可接受):障碍物=高亮句**全部行盒**(sent 第 5 参,Creature 从 readRange.getClientRects 全收集——多行句只让开末行会被上半身压住前面满宽行);候选序 整句右侧→左侧→**正下方**(多行满宽句主位)→**正上方**(句贴面板底)→全撞句时最小遮挡+`occluding=true`(渲染层 cp-occluding 半透明);返回 `{x,y,dir,occluding}`,dir 四向驱动姿势(point/pointr/**pointu** 抬臂 172° 指上/**pointd** 外展 -38° 下指,CompanionPose 扩两值);记笔记落点通道 `setLastNoteMark`(画线 mark→飞线旁+本笔道具 cp-writing/cp-pen;v11.1 文本自愈:mark 节点被 React 重渲染换掉后按画线文本重找回,治"加笔记后 bot 消失");打字=臂内收敲身体+胸屏显字 `lastKey`/`cp-screen-key`;肩甲 v11.3 已按用户反馈移除(`Shoulders` 组件删除,五形态+霜绒冰棱全拆);`data-zone` 混合(op 态=bus zone[headless 安全],roam 态=dispZone——rAF 驱动值在隐藏窗口会冻死,ui-test 实测踩过)。⑥**v9.1 三修**:rAF effect deps 补 `enabledLoaded`(否则设置异步加载后 deps 不变,循环永不启动=左上角卡死,ui-test 位置探针锁定该回归类);`speech-text` HARD/SOFT 全角标点 typo 根修(！？；，：从未进集合——v0.x 潜伏,中文 ！？ 不切句+全角逗号非软标点→超长硬断,即"高亮句中断开"主因);**TTS 块≠显示句**——`endsWithSentenceEnd`/`groupSentenceChunks`(shared)把强制断句块在渲染层并成显示句组,NotebookPanel/ChatStream 高亮整组、组切换才重标;ui-test 窗口 `backgroundThrottling:false`(隐藏窗口暂停 rAF,位置探针曾读到假冻结)。**v11 形态平等化+桌宠**:frost/moss/astro/ink 补齐 SS/L/FV 真辅音嘴型+主题 BevelPlate(`coarseViseme` 删除);`pickRoamIntent` 目的性游走(复习徽章/下节课球/卡点回访,~1/3 桶,`INTENT_HOLD_MS` 3s);`correctStreak` 3 连对 flame+`coreLit` 金环;`noteTick` 金勾(笔记 tab `companionNote` + 卡点毕业经 `companion-state-changed`→getDashboard 检测);**桌宠模式**=`companion_pet_mode` 设置→`pet-window.ts` 透明置顶穿透窗(`setIgnoreMouseEvents` forward 热区切换,`companionPet:setClickThrough` 通道,`pet.html`/`PetCompanion.tsx` 双页 vite input,bus `petMode` 主窗隐身);听写胸屏 `cp-screen-wave`;栏矩形 150ms 节流。verify T20(意图/情绪/记忆纯函数)+T21(桌宠接线守卫)。**v11.1 四修**:karaoke 句匹配 getTextModel 加 `includeMarks` 选项(markReadingSentence 用——画线 mark 的文字必须计入句文本,否则含画线的句子匹配失败→加完笔记高亮熄火;持久画线偏移侧仍跳过 mark 保持对称);拖选「提问/加笔记」按钮 250ms 尾随防抖→80ms 节流(按住拖选途中即现);rail 滑翔 glideTo 包住物理刚体坐标(旧:DOM 先写一拍,物理下一帧拉回=跨栏回栏瞬移)。**v11.2 表意分隔**:`speech-text` 换行/emoji 都是句界(整行成句、句尾 `\n` 作显示终点标记;emoji 序列含 ZWJ 连写/VS16/肤色修饰整体成句;CRLF 归一 LF),显示组另加 `DISPLAY_GROUP_MAX`=160 上限——治"无标点段落被并组吞成整段、高亮铺满到结尾";verify T9+T10(单一入口守卫)。**v11.2.1 emoji 句匹配回退**:matchSentenceAligned 对规范化为空的句子(emoji 独立句/ZWJ/VS16 序列/省略号句——表意句界的产物)回退**原文逐字查找**(canonical 只留文字字符,emoji 句 canon="" 旧逻辑必 null → 高亮冻结在上一句=用户实测"高亮慢一句"的根因;插桩定谳法:页面劫持 AudioBufferSourceNode.prototype.start 记音频起播时刻 + CSS.highlights.set 记高亮切换时刻,两张时刻表对齐看,修复后每句 ~80ms 内切换);verify T16 ⑥(UTF-16 代理对偏移直测)。**v11.4 权威原文直挂**:ttsAudio 事件携带 `sentence`(该块朗读原文,合成侧唯一真源),useSpeech 登记 `streamTexts`(ref 供 pump/state 供 karaoke)+`playingSentence.text`;NotebookPanel/ChatStream karaoke 一律 `playedSentencePrefix(streamTexts, idx)`(shared 纯函数:向前收拢到最近句终点块,拼**已播**前缀——强断句块并回同句且只亮到进度,高亮随音频逐块生长)——**渲染层零复算句表**(旧"从 content 复算+组号映射"两侧分叉之源,净化差异/翻译切换/参数漂移都会错句;用户两轮实测不同步后的根治);verify-speech-split T10(渲染层禁 speechSentencesOf/内联切分,T10 变为架构守卫)+T11(前缀纯函数)。**v0.17.2 精细化**:表情全收进头部屏幕(五形态 `cp-scr-face` 屏幕剪裁包 Face+FaceExtras,思考眉入屏 y55/口腔上收 y≤86,毕业金冠=头顶实体在剪裁外)、嘴型机器人化(舌=发光音素条 p.pupil/齿=分段刻度/咬唇=快门压光,五形态同契约)、喷焰粒子推出身形半径外做定向尾焰(旧版距旋转原点仅 ~15-23px 被身体盖死从未可见)+焰长随 --cp-speed 伸缩、**点球互动**(`companionBallTap` 物理球/键盘球/搜索三路发射→ref 级 BallTap 注册表 rAF 直读;rail 在场飞球右肩+轻顶非锁定球,T3 无 rail 在当前栏朝左注目礼;表情爆发复用 nodePoint);verify T24 守全链。**键盘反馈包**:`sideFromCode` 真实键位(QWERTY 左右半区→左右臂,未知键交替)/胸屏信号面板(`scopeBars` 确定性脉冲条+字符读数,Enter→/退格⌫闪发 keyFlash)/击键 squash(wrap class 重触发)/爆发专注(KEY_BURST_MS 3s 内 6 键→thinking,每键续期)/**暂停相位** `pausePhaseOf`(1.2~6s listening·6~15s thinking,存 `state.pausePhase` 让 tick 提前返回能感知相位转移;lastPress=0=从未打字哨兵)/backStreak≥3→encourage 汗滴/compositionend 汉字上屏;verify T25。**v0.17.1 质感五件**:①mascot SVG `overflow: visible`(旋转/抬臂越界不裁剪);②中/右栏投影=CSS `drop-shadow` 整身投影跟随剪影(右下偏移对左上光源,纯合成器零逐帧;`GroundShadow` 椭圆地影组件+`--cp-alt` 高度感链路全删,左栏天空无影);③喷焰速度驱动(Creature rAF prevPosRef 算帧速→`--cp-speed`(0..1)/`--cp-thrust-deg`(焰朝向=运动反方向) CSS 变量,静止即熄,不再只绑 flying 姿势);④五形态 `<Arms>` 移到 cp-head 之后(抬臂不被头盔遮);⑤手机工具栏图标化(coarse 隐藏 `.tb-label`/`.tb-chevron`,ModelPicker 补 `.coarse-only` Boxes 图标,`.composer-toolbar` nowrap 单行——修 40px 命中把文字钮撑爆换行的回归);T15 有五件标记(含 lastIndexOf("<Arms")>indexOf("cp-head") 逐形态图层序断言)。
  - **竖线眼**(用户拍板,EVE 式冷萌):五形态全表情统一竖圆棒词汇——`forms/shared.tsx` Face 眼区重写为 `bar()` 圆头竖棒助手(旧圆角矩形瞳孔眼/横线睡眼退役),情绪=棒的长短/粗细/浓度(常态 13 双棒、惊 18 拉长收细 4.5、思考左短 9 右长 13 不对称、听 12.5、睡两粒 4 短淡点 op .55);竖线的两种变形按用户点名:笑(happy/cheer/proud)=竖棒弯成上拱弧 `^_^`、鼓脸 huffy=竖棒旋横 `-_-(h12)`;星星眼=竖棒本体+棒旁小星芒。**视线跟随=整眼平移**:棒渲染进 `cp-pupils` 组(星/横线/默认三分支),壳的 pointermove→lerp 机制零改动继承,幅度 3/2px→**5/3px**(竖棒比旧瞳孔大,小位移看不出);眨眼=外层 cp-eyes `scaleY(0.14)` 压扁竖棒成短点(实测组合无冲突;口型/打字链不碰眼睛)。verify T15 守 bar() 助手+^_^/-_- 变形串+pupils 三分支计数+5/3 幅度串。
- **CompanionPack 自定义纸偶(v0.31,第 6 形态 custom)**: 用户导入立绘 PNG → `cutFromImage` 切分成分件包 → 作为伴学第 6 形态渲染,**行为系统零改动**(Mascot/Creature/bus/flight 全零 diff,G1 守卫名单;姿势/拍臂/口型/朗读跟句等行为天生共享)。切分降级链=多模态识图折线(headBoundary/臂缝)→几何臂带(3-run 行包络)→L1 整图;部件加白描边(原始 alpha 盒模糊 r2→阈值 108→分离膨胀,治 AA 逐列抖动被膨胀放大的轮廓台阶,T26 锁)。布局=parts 并集 figureBox 原样 contain + **底部锚定**(任何纵横比素材脚都踩载具顶面,G2f;不得混回画布宽高)。臂 **rest 角补偿**(§17.12):姿势固定角隐含"臂竖直 90°",用户 PNG 臂任意 A-pose 必指偏——`armRestAngleDeg` 从臂部件像素量"肩原点→手尖"方向,姿势 CSS 按 `rotate(calc(目标角 − var(--cp-rest)))` 求差;选择器必须复合写法 `.cp-form-custom.cp-pose-point`(后代组合器永不命中——两类都挂 svg 根,实测踩过,G2h 全局禁)。悬浮载具=程序化 SVG 机械平台(顶/壁/底三层有厚度+前沿仪表舱接纳壳层胸屏内容+三喷口 idle 微焰/飞行加力),**五款形态主题**(`renderer/lib/companion/veh-themes.ts`:小焰熔岩橙/霜绒冰蓝/苔芽绿/星尘紫夜+星点/墨墨漆黑+朱砂;颜色全内联 TSX,CSS 只持动画——类规则 fill 会压过 SVG 表现属性)存 `manifest.vehicle` 持久化;向导 step3 选型+形象栏卡片色点即改(`companionPack:setVehicle`,激活包即时换装)。**多 bot**:包存 `userData/companion-packs/custom-<hash8>/`(内容哈希多实例,重导同图零 LLM 幂等),设置页包卡(色点换载具/×删,ConfirmCard 确认)+「+」进三步制作向导(①原图指引不上传 ②一键复制管线友好 chibi prompt(特征锁定+二头身规则+A-pose 比例+绿幕条款,双语在 i18n `companion.wizard.prompt`)+豆包/即梦/ChatGPT 直达 ③导入→切分预览→命名保存即用)。协议 `companionPack:cutFromImage/applyPack/getActive/list/activate/delete/setVehicle`;识图 locate 走 `buildImportModel`(与导入同源纪律,G2m 守——裸模型无 thinking 字段,glm 系端点默认开思考:桌面 184s/手机 2s 空回复,2026-09-12 真机定谳);verify-companion-pack(G1-G5)+verify-companion-cut(T1-T26)双套守。PNG 像素进出**双后端**(`services/pure/png-codec.ts`:napi 优先、pngjs 纯 JS 兜底——Android 无预编译,Termux 手机本机可跑完整向导;缩略图统一 resizeBox 面积平均,全平台逐像素一致;LOOKATSTUDY_PNG_BACKEND=pure 强制纯 JS);verify-companion-png T4 同 fixture 双后端全链对拍。
...
- **栏宽拖拽 (issue #14, `lib/pane-resize.ts` + `PaneResizeHandle`)**: T1/T2 在场栏间边界可拖调宽——左栏 `pane_width_left` [240,480]、中栏 `pane_width_mid` [480,1100](settings 持久化,空=未定制=响应式默认),右栏恒 flex-1 吃剩余。钳制**视口感知**(`clampRail/MidCandidate`:三栏皆 shrink-0/flex-1,宽度和超视口=右栏被顶出屏幕,flex 兜不住;`solvePaneWidths` 渲染期把旧定制宽压回当前视口预算,中栏硬底 320);拖拽中宽度 imperative 直写目标元素(**不逐帧 setState**——左栏物理地图整树/中栏消息流重渲染不可接受,左栏岛 ResizeObserver→重建按 100ms 节流跟随),松手才提交 React 状态+持久化;中栏 width transition 拖拽中置 none;双击手柄重置。手柄命中区触屏 44px/细指针 12px(index.css),`useViewportWidth` 仅定制宽度存在时订阅 innerWidth(档内 resize 不全 app 重渲染,与 useWindowTier 同哲学)。verify-pane-resize 纯函数+源级守卫;ui-test 合成 PointerEvent 异步分段派发(pointerdown 后必须等 React commit,window 监听才挂上)。
...
npm run serve             # dev serve: esbuild server bundle only + serve dist/renderer (web 模式调试;默认 --data .serve-data 一次性目录,传任何参数都不丢该默认——旧版带 --port 就静默落回 ~/.lookatstudy 持久目录)
npm run build:mobile      # 便携包 dist/mobile/: server.cjs 单文件 + web 前端 + install-termux.sh(Termux 手机端)
npx tsx scripts/live-test/live-test-epub-corpus.mjs # EPUB 拆分健壮性语料核查(8 本结构各异 Gutenberg 真书;下载缓存 scripts/fixtures/epub-corpus/,gitignored;断言标题-内容对齐/license 零残留/章数带宽)
npx tsx scripts/live-test/live-test-pdf-corpus.mjs     # PDF 提取样料核查(6 真样本+合成扫描件;部首区零残留/正文量/扫描诚实空;缓存 fixtures/pdf-corpus/)
npx tsx scripts/live-test/live-test-web-corpus.mjs     # 网页正文抽样核查(5 站点;尾部站点模板零残留;知乎 403 诚实报错断言)
npx tsx scripts/live-test/live-test-subtitle-corpus.mjs # 字幕成文核查(freeCodeCamp 真人翻 srt;滚动窗去重/CJK 接行零污染)
...
npm run ui-test           # real-GUI verification (headless Electron, 72 DOM assertions;incl. shimeji 第七形态四断言(importZip→角色清单→confirmImport→list→activate→getFrame 协议全流程/设置页第七形态选择项+区块+导入卡/形态切 shimeji→cp-form-shimeji+帧渲染器挂载/删包回落)+incl. companion-pack M2 全链(cut→apply→渲染→行为复跑→持久化→按 id 删回落)+incl. companion v3 single creature (rail home zone after course pick + composer focus flies to chat zone w/ win.focus() for real focus events + read-aloud talking → notebook zone + T3 pane-switch persistence + form live-switch) + a11y + reactive i18n + cold-start gating + empty-start course gating (no auto-select, manual pick, delete→empty-state) + boot guide (引导屏出现/三问向导走完入库+boot_done/不重播/resume_last 一键恢复/右栏回顾两态) + course search (tree nav + title filter + jump) + start-learning action-label bubble (no prompt leak) + seed bilingual 🌐 switcher + post-reveal choices + competence badges + due/interleave/dashboard + start-here cue + exam answering integrity (option display order matches grading, click correct text → 3/3, long prompt stays in viewport) + voice tiers (mic click→voice mode→hold→release→review/error panel→back to keyboard, 设置页语音按钮组(朗读 Edge/本地/系统/自定义,听写 本地/自定义)+ whisper 模型下拉 + 讲解 🔊) + shiki 代码高亮(讲解区代码围栏 .shiki 节点 + 双主题 CSS 翻转 computed color 变化))
npm run lint              # oxlint
...
npx tsx scripts/live-test/live-test-review-session.mjs # LLM 复习会话行为测试(五轮真实会话:开场先忆后问且对准上轮薄弱点/测验真调 generate_quiz/收束真调 end_review_session 且 quality 合法/全程不调 mark_mastered)
...
|---------|------|-------------|
| Agent engine | `services/agent/agent-engine.ts` | `handleAgentChatThread` — thread-based context assembly, 6 display tools (`show_concept_map` / `generate_quiz` / `compare_table` / `draw_diagram` / `show_code_walkthrough` / `pose_guess`), `chat:part` emission, mastery-based teaching strategy; 注入近期 friction 卡点(`pure/friction-context.ts` buildFrictionContext)让 AI 看见学习者挣扎点;**AI 输出语言 = 界面语言**(i18n,非课程 🌐):渲染层穿 locale → `resolveOutputLang`(`@shared/locales`,纯函数)→ `agent/base-prompt.ts` 组装提示词(zh 中文本体逐字节回归锁;非 zh **英文本体**——2026-08-31 i18n 落地,与中文版三级分层逐段同构,非旧版"中文约束+一句英文指令";「[工具调用已执行]」标记字样保留中文原样,系统注入格式不翻译);语言学习双轴(v0.33,课程 `language_target` 非空=导入时 LLM 判为语言课):目标语言 carve-out + 【语言教学姿态】块注入基座——教学语言=界面语言,题目语言素材=课程目标语言原文(issue #15 学英语被翻中文出题的根);exercise/exam 出题同一链(`questionLanguageLine` 双轴);**v0.39 复习会话**(threads.kind=review):`assembleContextBlocks` 注入【复习导师姿态】(`base-prompt.ts` `buildReviewTutorBlock` 双语,先忆后问/错了才讲/收束标尺)+ 注册 `end_review_session` 收束工具(quality 1~5 喂 SM-2+XP+streak,**不写 BKT**;双重防重复=回合硬闸+`shared/review-session.ts` 按 `[[review-kickoff]]` 标记的轮次判定;context-usage 同源吃 reviewMode) |
...
| LLM client | `services/agent/llm-client.ts` | `resolveLlm` (3 protocols), `testLlmConnection` (主模型/视觉覆盖双路,回显延迟 ms), `classifyLlmError` (auth/rate-limit/network), `fetchOpenRouterModels`, `fetchProviderModels`。openai-compatible 协议分家:官方 OpenAI 端点 `createOpenAI`(原生 providerOptions),第三方端点 `@ai-sdk/openai-compatible`(chat/completions 解析 `reasoning_content` → reasoning-delta,思考流进 UI;includeUsage 与官方包对齐)。v0.38 预设分支 models=策展∪overlay(`presetWithOverlay`),supportsVision 查表能看见 overlay 添加的模型 |
| LLM presets | `services/agent/llm-presets.ts` | 19 provider presets (GLM standard/CodingPlan, DeepSeek, Kimi, Qwen, SiliconCloud, OpenRouter, OpenAI, Anthropic, Google, Groq, Together, Mistral, xAI, Volcano, Baidu, MiniMax, Baichuan, StepFun);2026-09-12 全量刷新(z.ai /models 实测 + OpenRouter 445 型实时返回,低置信处注明)。策展条目=合并真相层:**目录只补元数据不改可见性、填充不覆盖**(null 才填,窗口/价格/能力永不翻案策展值) |
| 模型目录供给链 | `services/agent/modelsdev-map.ts` + `model-catalog.ts` + `model-overlay.ts` + `lib/model-catalog-refresh.ts` + `scripts/build-model-catalog.mjs` | v0.38 元数据三层合并:user overlay(预设 `model_overlay_json` / custom modelsJson)> 预设策展 > 目录(运行时刷新缓存 `model_catalog_cache` 新者胜,否则构建期快照 `src/main/assets/model-catalog.json`——models.dev api.json 生成,16 家映射 757 条,离线常备;快照资产三触点=vite emit 插件+build-server beside+seed 同款多候选 walker)。映射表单源 `modelsdev-map.ts` 生成脚本与运行时刷新共用;`resolveModelMeta`/`effectivePresetModels` 是渲染层与两个收口(resolveActiveContextWindow / getProviderPresets)的统一出口;快照缺失=空目录优雅降级(增强层永不阻塞启动)。发现:`agent:discoverModelsFor`(key 主进程侧解析,返回条目已做目录回填)→ 设置页 ModelDiscoveryPanel 勾选添加/手工直填/移除(overlay 纯变换在 `@shared/model-overlay`);另有 `agent:getModelMeta`/`getModelOverlay`/`setModelOverlay`(learner_profile 同款归一化闸门)/`refreshModelCatalog`(24h 缓存,失败静默用快照)。渲染层消费:ModelPicker 价格/推理/免费徽标+deprecated 淡化、ContextMeter 单价行、EffortPicker 目录诚实态(reasoning=false 禁深度档)。verify-model-catalog T1-T8 |
| 模型管理弹窗 | `renderer/components/ModelManagerModal.tsx` + `ModelVisionTab.tsx` + `Toggle.tsx` | v0.38:设置抽屉的「AI 模型/看图模型」区整体迁入专属弹窗(App 根层 lazy 挂载,PersonalProfileModal 同款骨架:遮罩+role=dialog+useFocusTrap+自有 Esc),抽屉换两张紧凑卡(模型卡=当前 provider/模型+元数据徽标+看图状态行,整卡可点;看图状态卡)。顶部分段 [主模型][看图]:主模型 tab 双栏=左列表(搜索/预设+custom/激活·key 两态点;**点选=浏览不切激活**)+右详情(key 失焦即存永不回显/模型下拉**选定即激活**/发现面板/测试);看图 tab=原 MultimodalContent 整体迁入(testid 全不变)。五入口:抽屉两卡/ModelPicker 底部(不再绕抽屉)/Composer 无 key CTA/keyless 卡/开屏 settings_llm——props 名 `onGotoSettings`→`onOpenModelManager`(App state 下传,家规不用 CustomEvent)。浏览非激活 provider 也要能测 → `agent:testProvider`(key 主进程侧解析)。旧「保存 AI 配置」页脚退役(即时生效);openrouter 旧会话态发现路径退役。SettingsView 净删 ~600 行;ui-test T18b + verify-model-catalog T8 守;verify-pdf-math-vision/secret-handling/pane-tiers 断言已重定向 |
| Vision bridge | `services/agent/vision-bridge.ts` | 图像转译桥(v0.11,describe-then-chat):`visionRouting`(native/bridge/reject)是**三处喂图点共用的通道路由**——① 聊天附件注入 ② 课文图主动注入(方案 B,`flag_multimodal_import` + `isImageRelatedQuery` 门控)③ `attach_node_images` 工具。native=主模型直看 file-part;bridge=纯文本主模型 + 配了 `vision_provider_override`/`vision_model_override` → 视觉模型(`resolveVisionLlm`)转译成**不可信视觉证据**文字注入(工具则返回转译文本;reject 通道下工具不注册、方案 B 不喂图,修掉纯文本模型硬吃 file-part 的 400 坑);主模型永远是唯一大脑。学习者原话原样转发(任务导向转译);sha256(图+问题+语言) 进程内缓存(FIFO 200);watchdog 120s/5min;失败带指引报错绝不静默丢图;`parseDataUrl` 统一图源归一化(纯 base64)。`agent:getContextUsage` 的 `visionCapable` 桥感知 + `visionBridgeModel` 驱动输入框转译提示;设置页视觉覆盖不被 `flag_multimodal_import` 门控。v0.16 `isImageRejectionError` 拒图措辞识别 + `agent-engine` 400 保险丝(首 part 即错时自动降级重试一次,桥接/不喂图,附可见提示) |
| npm 分发(Termux 双包) | `build-termux-voice.mjs` 产 `lookatstudy-termux-voice` + `android-build.yml` 产 `lookatstudy-mobile`,**trusted publishing(OIDC 零 token**;需 npmjs.com 一次性配 pending publisher,npm≥11.5.1)发 npm,npmmirror 自动同步;安装与升级(`update.sh` 生成体与安装器逐字同源)**同一条下载链**:npmmirror 主源(+滞后守卫)→ npm 官方源 → GitHub 直连 → gh 代理;解析函数恒 exit 0(`set -e` 契约,miss 返回 1 会杀脚本),verify-termux-voice T7 守链序/双脚本 bash -n/契约 |
...
| TTS 朗读+剧本口型 | `services/speech/tts-service.ts` + `tts-tiers.ts` + `edge-tts-client.ts`(v9 saveSubtitles 逐词 sidecar) + `viseme-script.ts`(v9 词时序+拼音→SpeechVisemeCue) + `azure-tts-client.ts` + `tts-cache.ts`(v9 cues sidecar) | v0.15 档位:**edge 在线(默认,免费无 key,node-edge-tts→mp3;Windows 需 ipv4first DNS 调谐)** / local kokoro / custom-<id>(OpenAI 兼容 /audio/speech,`openai-tts-client.ts`,音色=tts_custom_voice 可空,测试=真实合成一句);v0.18 第四档 **system**(浏览器/系统 speechSynthesis,显式自选不进自动降级链:渲染层第二播放管线`useSpeech` 逐句 utterance→playingSentence 驱动 karaoke/伴学,句切分共用 speechSentencesOf,口型时间轴不可用=嘴型静默;音色挑选 `renderer/lib/system-tts.ts` 纯函数偏好名→中文→default,WebView 无该 API 置灰;档位缓存 window 事件广播);azure 为旧库遗留仍解析(UI 无入口);edge 句级预取(深度 2)+ **失败自动落 local**(fellBackTo 回执),azure 失败即报错不静默降级;缓存键=sha256(engine\|voice\|speed\|sentence)(custom 档 engine=provider id、voice 段含模型)按容器分 .wav/.mp3;首次 edge 用回执 firstUse 驱动一次性在线披露;事件载荷加 mime。讲解 tab 标题旁 🔊 整课朗读(`NotebookPanel` useSpeech 实例 id=content-{nodeId},切节点即停,markdown 净化复用 normalizeSpeechText) |
...
| Threads | `services/thread-service.ts` | CRUD for `threads` + `chat_messages`; `findRecentThreadByNode`; thread is node-bound (`focus_node_id`);**v0.39 `kind` 列**(chat/review——复习会话线程,引擎按它注入复习导师姿态,ThreadSwitcher 加 Repeat 徽标)。`chat_messages.display_text`:按钮触发的消息气泡只显示短动作标签(完整提示词在 `content`,只给 LLM;手打输入 `display_text=null` 原样展示) |
...
| Highlight | `lib/highlightText.ts` | 画线定位:getTextModel + 文本搜索(applyPersistentMarksByText)+ 跨节点包裹(wrapRangeWithMark)+ 闪烁(flashMark)。**不依赖 DOM offset**(ReactMarkdown 重渲染不稳定),用 indexOf 在纯文本上定位;getTextModel 跳过 `[data-md-diagram]` 图卡子树(mermaid 围栏图卡不进画线/朗读跟句文本) |
...
| Custom providers | `services/custom-provider-service.ts` | BYO user-defined provider rows; bypass preset settings, resolved by `custom-` prefix。v0.15 `kind` 分区(llm/vision/tts/asr,老行默认 llm):主模型区/主模型快切(ModelPicker)只列 llm,看图/语音各区只列自己的;`tts_engine`/`asr_engine` 取值可为 `custom-<id>`(active_provider 式)。v0.16 `vision` 列:kind=llm 行用户显式勾选 → `resolveLlm` 合成 models 条目含 vision capability → `supportsVision` 统一查表决定 native/bridge/reject |
| Serve runtime | `src/main/serve/server.ts` + `serve/index.ts` | 手机/浏览器模式:同一 handler 表的 WS 分发 + 静态前端 + token 鉴权(4001 拒连);启动序与 Electron 主进程对齐(initDb→seed→souls→prefLang),CLI `--port/--data/--web/--tls-cert/--tls-key`(TLS 成对提供→https/wss,解锁 LAN 安全上下文;渲染层 `installWebApi` 落 `__lookatstudyWeb` 标记,桌宠入口/主窗隐身在 web 运行时自动失效) |
...
| Web transport | `src/renderer/lib/api-web.ts` + `shared/api-channels.ts` + `ws-protocol.ts` | 浏览器版 window.api(WS req/res + event 帧,断线重连,4001 不重连);125 方法↔channel 映射自 preload 生成,verify-serve T5 守漂移 |
...
| Context usage | `services/agent/context-usage.ts` + `shared/token-estimate.ts` | 输入框上下文表(v0.10):`agent:getContextUsage` 返回固定开销(系统提示/课文/学习者快照的启发式 token 估算)——装配抽 `agent-engine.assembleContextBlocks` 与实发同源不漂移;渲染层本地叠加对话历史+草稿(`estimateTokens` CJK 感知纯函数,窗口 = `resolveModelContextWindow` 统一解析:预设表 → 自定义 provider modelsJson 条目(设置页可编辑,OpenRouter 发现自动带回 context_length,其余家 /models 不含窗口需手填)→ null 诚实未知;窗口解析统一出口=llm-client.resolveActiveContextWindow) | v0.26+ **历史预算裁剪**:runAgentTurn 装配时对历史按 预算=(窗口-4096)×0.6-固定开销 从最新往回保留(`shared/history-budget.ts` 纯函数+verify-history-budget;minKeep=2 当前问题与最近一轮永不被裁;窗口未知保守 32k;裁在 preparedMessages 构造前→fuse/降级零漂移; 渲染层用量表不裁,读到略高于实发的保守值) |
...
| Reasoning effort | `shared/reasoning-effort.ts` | 思考强度方言表(v0.10,存 `settings.reasoning_effort`:""自动/fast/deep):GLM→body.thinking.type、DeepSeek→同款对象 {type}(2026-09-21 修 issue #17:初版字符串 "max" 被 serde 拒 400)、Qwen/SiliconCloud→enable_thinking(经 `llm-client.buildLanguageModel` 的 fetch 覆盖注入)、OpenAI→reasoningEffort、Anthropic/Google→providerOptions;支持的家族:GLM/DeepSeek/Kimi/豆包/StepFun(布尔/对象/字符串参数按各家方言)/xAI(按模型门控,裸 grok-4 传参报错自动落 none);不支持的家族(MiniMax 官方无开关、百度预算式形状特殊)芯片禁用+引擎降级 none,宁可不生效不瞎发参数。**家族嗅探必须传 hints**:UI 门控(supportsReasoningControl)与引擎调用点(reasoningPlanFor)都按 baseUrl/模型名嗅探 glm-codingplan 预设与 custom-*(2026-08-18 修:引擎调用点曾漏传 hints,fast 对它们静默失效——开了快速仍思考 9187 字);verify T6d 是源码级接线守卫 |
...
| URL import | `services/url-import-service.ts` + `pure/url-route.ts` + `pure/html-article.ts` + `pure/text-chunk.ts` | 智能链接导入:`routeImportUrl` 四分流(github/arXiv(新式+旧式 ID)/article/video(B站 BV/av/分P/b23.tv 短链 → bilibili 源;youtube/youtu.be 与抖音 douyin.com/v.douyin.com/iesdouyin.com → ytdlp 源,导入 job 内二次分流));`extractArticle` = linkedom DOM + @mozilla/readability 正文抽取 + turndown 转 markdown(纯 JS 零原生;标题保留、img 相对地址绝对化、非文章页诚实返回 null);`stripTailNavigation` 尾部**站点模板指纹**清理(2026-08-23,5 站点采样:搜狐返回行/阿里云侧栏/行内导航后缀/裸图路径行,尾窗 25 行+markdown 前缀感知——规则只删跨文章稳定的机器模板,作者推广段是正文不碰,尾部语义边界交 Step4 LLM);`fetchArxivMarkdown` = abs 页取标题(`extractArxivTitle` 纯函数:citation_title meta 优先 + `<title>` 剥 `[论文号]` 前缀兜底——旧贪婪正则会回溯到只剩一个字符,已修)+ PDF 下载(`downloadToBuffer`)走 `parsePdfText`(扫描版报错不硬吃);`text-chunk.ts` = 无标题长文的句子边界预分段(粘贴/arXiv/(M3)转写共用,`prepareSingleDoc`:≥3 个 H2 或 ≤8000 字整体单文件,否则分段) |
| Video import | `services/video-import-service.ts` + `pure/bilibili-wbi.ts` + `pure/subtitle-parse.ts` | 视频链接导入(虚拟源 kind=video,身份=归一化URL):**B站直连** = wbi 签名(`bilibili-wbi.ts`:nav 取 img/sub_key + MIXIN_KEY_ENC_TAB 重排 + MD5 w_rid,免登录)+ view 取 cid/标题/分P列表 + playurl(逐P) DASH 最低码率音轨;**B站多分P整季**——URL 不带 `?p=` 导全部分P(audio-multi:每P逐段转写各成虚拟文档,Step4 按集分章,200P 封顶),带 `?p=N` 只导该集;**同 URL 重导早退**——已有快照带 docCache 直接恢复,零拉流零转写(Step1 现拉铁律在视频源上的例外,整季重转写代价过高);**YouTube/抖音/千站** = yt-dlp 检测已装(PATH 探测)+**字幕优先**(`--write-auto-subs` → vtt/srt 经 `subtitle-parse.ts` 剥时间轴成文本,零转写零模型;**成文三修(2026-08-23,freeCodeCamp 真人翻 srt 语料)**:滚动窗去重(YouTube 自动字幕 cue N+1 首行复述 N 尾行,近窗 2 去重且基于原始行)、CJK 感知接行(汉字间不加空格,旧实现全文空格污染)、`pickSubtitleFile` 多语言取件中文优先(readdir 字母序会让 en 压过 zh-Hans);B站 CC 字幕 API 免登录返回空(需 SESSDATA),维持音轨转写),无字幕才 `-f bestaudio` 下载;未装 yt-dlp 给平台安装指引文案。音频解码走 audio-file-decode(fMP4 兜底见下行);docCache 随快照落盘断点续跑。live-test 真实B站全链(Rick Astley MV: 拉流→fMP4→ADTS→解码 212s) |
...
| Exam | `services/exam-service.ts` + `exam-generation-store.ts` | 章节考试 v2:KC 分批后台出题(真实进度,`exam:status` 事件)+ `regenerateExam` 重新出题(删旧题/在飞 no-op/悬挂判死/历史星数保留;判分快照 prompt/options 让历史回顾自包含)+ attempt 档案(`exam_attempts`,逐题增量持久化/悬挂自动判死/terminated 未答=错);`shared/exam-logic.ts` 纯函数(题量**覆盖优先 clamp(KC,3,15)**——每 KC 恰一题,>15 等距采样首尾必选,<3 凑下限;2026-09-19 拍板"题不在多而在精准"/v0.19 每题限时动态:45+cjk/5+词/3+选项×8+代码25+公式25,clamp(60,300)/attemptId 种子重排题序+选项序);不回写 BKT。**考试范围三修(2026-09-19,用户实测考点超纲)**:①出题前补齐本章无 KP 课时(`ensureSectionKcs` 3 路并发复用 generateLessonSummary,失败不阻塞伪 KC 兜底;同步前缀不变量=补 KP 的 setGenerating 先于首 await,否则 setPromise 挂不上)②出题上下文加厚:摘要前置+正文 800→1500 字 ③prompt 章节围栏(只考察提供内容中出现的概念,禁止引入其他章节);verify-exam T2 钉新配额+T18 三修源级守卫 |
...
| 课程推进边界卡 | `shared/next-lesson.ts` + `lesson:mastered` 事件(ipc/index.ts 双过渡点) + `App.tsx` 边界卡 + `MapRail` next-cue 环 | 一课**首次**毕业(`proposal:apply` 的 mark_mastered 与 `quiz:recordAnswer` 两处 `!wasMastered` 过渡,与记忆固化同口径)→ 对话区出卡;点「开始下一课」伴学飞下一球旁指向(companionWhistle+NodePoint)+金色虚线呼吸环常驻到点球(render 时挂类,物理解锁重建岛不丢)+T2/T3 强制左栏/T3 切屏(handleBootPickCourse 同款活路,**不走 setView 死线**)——bot 指路、用户开车,不自动切节点;下一课排序=`nextLessonAfter` 纯函数(同段顺延→下段首课跳空段),agent 不参与排序;最后一课=终点态卡(无按钮 10s 谢幕);base-prompt 【课程推进边界】条款同弹落地(禁"说继续自动开讲"空头承诺);verify-next-lesson 守 |
...
| SRS | `services/srs.ts` | SM-2 spaced repetition; `recordReviewDb`(pure/srs-db.ts, db 注入,**v0.39 返回 Sm2Result 回执**)与 BKT 闭环——答题/复习双向同步(答对推迟、答错近期重练);`srs:record` 与复习收束工具都 `touchStreakToday`(纯复习日不断连胜) |
...
| 开屏导师 | `shared/boot-guide.ts` + `src/main/services/boot-state-service.ts` + `src/renderer/components/BootGuidePanel.tsx` | 未选课态中栏的冷启动/回访引导：`computeBootGuide` 纯函数状态机（初次四步向导 + 回访九景，"继续上次/复习到期"优先于一切）；`gatherBootState` db 注入式一次聚合全部输入（`boot:getState` 单往返，含 resume 目标/卡点/快毕业/考试中断/streak 含 freeze 语义/隔天回归 + 右栏 recap 数据）；台词全本地 i18n 零 LLM；boot 向导 `boot_done` 看过即置位永不重播（向导走完或左栏选课离场两路都置位）；引导屏不是 gate，actions 一键出口 |
| 学习者画像 | `shared/learner-profile.ts` + `src/main/services/proposal-service.ts#update_learner_profile` | 声明侧画像（区别于 memory 推断侧）：MBTI 快捷入口展开 style 四维真源（start/interaction/feedback/pacing，16 型双语表+场景题兜底）+ 动机阶段 `motiveStage`（OIT 内化连续体 2-6 级五选一，白话外壳 `motiveDisplay` 零术语暴露；注入「学习动机」行+【动机适配】五级内化教练+双护栏——动机是连结器不是内容边界/闭嘴纪律不泄漏理论；**AI 不可提议改动机**——LearnerProfilePatch 与工具 schema 双层排除，goal/goalNote 已退役）+ 兴趣一等字段 `interests`（兴趣个性化挂钩素材：注入数据行「兴趣点：…」+ 搭桥条款，`parseInterestsInput` 共享解析，向导卡2/编辑表单双入口，AI 可经工具提议补充），settings `learner_profile` JSON（**未加表**），`profile:get/set` 结构化通道；注入提示词第④层（agent-engine profileBlock，独立于 node-bound 快照——无课也生效；防注入标注+合意困难条款+与 soul 冲突时人设优先的明写优先序）；AI 补画像走 `update_learner_profile` 工具 Propose→Apply（证据≥2 才提议；apply 前仲裁——提议发起后用户手编则判 stale 不覆盖） |
| 个人资料窗口 | `src/renderer/components/PersonalProfileModal.tsx` + `ProfileEditForm.tsx` + `lib/avatar.ts` | 画像显式中心：标题栏头像入口(首字母+nameAvatar 稳定底色)→ 弹窗三区(我声明的=ProfileEditForm 共享编辑 / AI 建议的=画像提议消费点 `profile:listProposals`,建议卡+历史 / AI 记住的=memory 三槽 `memory:listAll`+`memory:deleteSlot`,flag 默认关+一键开启)；画像提议不在聊天流出卡(ChatStream 只渲染 part-profile-suggest 指路静行),工具描述约束自然停顿发起 |
...
| PDF text | `lib/pdf-text.ts` | `parsePdfText(buf)` — PDF **文本**提取路由:优先 `@firecrawl/pdf-inspector`(预编译 napi-rs, layout-aware markdown — 标题层级 + 多栏阅读顺序), 失败/平台不支持(Intel Mac/WinARM 无预编译)回退 `pdf-parse`;`LOOKATSTUDY_NO_PDF_INSPECTOR=1` 强制回退;双出口统一过 `normalizeRadicals`(2026-08-23:pdf-inspector 对中文 PDF 输出康熙部首区字形 U+2F00-2FDF,"手"变"⼿"——检索/画线/朗读全断,逐字符 NFKC 归一;live-test-pdf-corpus 真样本核查)。**已知局限:纯文本层不解码数学公式**——密集页走下一行 vision 转写 |
...
| PPTX parser | `lib/pptx-parser.ts` | officeparser AST → `{markdown, images}`;每 slide 一个 `##`(讲者备注非标题随 slide), **表格节点 → GFM markdown 表格进正文(2026-08-23,真 PowerPoint fixtures 驱动:此前 table 节点整层被忽略表格文字全丢;竖线转义/全空表整张跳过/表不当标题)**, 现有导入管线自动每 slide 一节课。内嵌图片复用 pdf_page source(避 schema CHECK 迁移)。仅 `.pptx` |
...
| Concept map layout | `src/renderer/lib/cmap-elk-layout.ts` + `components/artifacts/ConceptMapArtifact.tsx` | 概念图 v2(v0.21,替代 v0.12 径向纯函数):elkjs Eclipse Layout Kernel 懒加载 chunk(动态 import,失败保占位不炸卡)——复合分组(可选 groups 字段,缺省 `clusterByAdjacency` 兜底聚类)+ 正交路由 + 组容器标题栏;**elkjs 陷阱:`hierarchyHandling: INCLUDE_CHILDREN` 把全部边 hoist 到根 edges 数组、`container` 字段声明坐标系**,flattenElkResult 用 containerOff 映射还原组内边绝对坐标(按树位置加偏移会把组内边整体错位)。皮肤=draw.io 词汇:同色系浅填充深描边五色色板(--cm-c0..c4 CSS 变量,暗色明度反转)+ 组容器低透明度 tint/标题栏条带(暗 .42/亮 .55)+ 边标签胶囊白晕;hub=最大度数节点加粗放大。verify-artifact-cmap-v2 守确定性/零重叠/组包含/**边端点贴节点边界**/边界防御(自环/悬空边跳过) |
| Mermaid theming | `src/renderer/lib/lazy-mermaid.ts` | mermaid `theme:"base"` + `themeVariables` 实时读 CSS 设计 token(暗/亮自适应);theme-changed 清初始化缓存,MermaidArtifact 监听同事件重渲染在场图卡。v0.21 ELK 布局:`loadMermaid` 注册 `@mermaid-js/layout-elk`(该包发布件**内联 elkjs 0.9.3** 644KB gzip 懒 chunk,与概念图 elkjs 0.12 无法去重,已知情接受双份);`renderMermaid` 喂 mermaid 前把 flowchart/graph 前缀改写为 `flowchart-elk`(`mermaid-elk-rewrite.ts` 纯函数)——**出题提示词与修复回路零改动**,只有渲染层改写;未注册 ELK 时 mermaid 静默退 dagre 并 console.warn(ui-test 用该 warn 做 ELK 生效哨兵) |
| Diagram repair | `services/agent/diagram-repair-service.ts` + `artifact:repairMermaid` IPC | v0.20.1 mermaid 语法自动修复(archify 思想:验证回执→带错定点修→限轮数):MermaidArtifact 渲染失败 → 带解析错误 IPC 回主进程 → LLM 修复(fast 档,buildImportModel+看门狗)→ 重渲。只对**原始**代码触发一轮(修复稿再败不循环),修复稿渲染成功才进 sessionStorage 缓存;LLM 失败 ok:false 不抛,守源码 fallback。draw_diagram 描述同步加图类型选型指南 + 标签引号规则(中文/括号 label 必须引号包裹,裸括号是渲染失败首因) |
...
- **Component primitives**: `ConfirmCard` (inline confirm, replaces ALL `confirm()` — never use native), `Toast` with `severity` prop (success/error/warning/info), `GlobalTooltip` (any `data-tooltip` element; desktop hover follows cursor, touch = long-press 500ms per Material spec; both viewport-clamped). Touch density: `pointer: coarse` media block in `index.css` lifts composer toolbar pills to ≥40px hit targets (fine pointers unchanged) — new touch UI must respect the 44px floor.
