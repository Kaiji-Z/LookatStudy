...
5. `ci.yml` runs oxlint + both typechecks + 89 verify suites + vite build + mobile bundle on every PR and push to main — never merge a red PR. `android-build.yml` (tag `v*` or dispatch with `release_tag`) builds `LookatStudy-launcher.apk` + `lookatstudy-mobile.zip` and attaches them to the Release.
...
| TTS 朗读 | `services/speech/tts-service.ts` + `tts-tiers.ts` + `edge-tts-client.ts` + `azure-tts-client.ts` + `tts-cache.ts` | v0.13 三档编排:**edge 在线(默认,免费无 key,node-edge-tts→mp3;Windows 需 ipv4first DNS 调谐)** / azure BYO key(SSML→riff WAV)/ local kokoro 兜底;edge 句级预取(深度 2)+ **失败自动落 local**(fellBackTo 回执),azure 失败即报错不静默降级;缓存键=sha256(engine\|voice\|speed\|sentence) 按容器分 .wav/.mp3;首次 edge 用回执 firstUse 驱动一次性在线披露;事件载荷加 mime。讲解 tab 标题旁 🔊 整课朗读(`NotebookPanel` useSpeech 实例 id=content-{nodeId},切节点即停,markdown 净化复用 normalizeSpeechText) |
| ASR 听写 | `services/speech/asr-service.ts` + `asr-tiers.ts` + `cloud-asr-client.ts` + `renderer/lib/useAsrInput.ts` + `silence-detector.ts` + `shared/speech-wav.ts` | v0.13 质量优先管线(流式退役):渲染层录完整段(全量缓冲+RMS 音量条)→ 静音自动停(可关)/手动停/PTT 松开 → 客户端 WAV 编码(shared/speech-wav,16kHz 单声道)→ 一次 `speech:asrTranscribe` 换全文。路由:**local=Whisper 离线(turbo 优先,自带标点,decodeAsync 串行化防 CPU 互拖;zh 输出经 opencc-js 繁→简归一)** / groq whisper-large-v3-turbo(**复用 LLM preset 的 groq_api_key,零额外配置**)/ azure STT BYO。PTT:按住 🎤 ≥400ms 即录松开即停,快点击仍 toggle(自动化 .click() e.detail=0 走 onClick);"静音后自动发送"默认关 |
...
| Web transport | `src/renderer/lib/api-web.ts` + `shared/api-channels.ts` + `ws-protocol.ts` | 浏览器版 window.api(WS req/res + event 帧,断线重连,4001 不重连);100 方法↔channel 映射自 preload 生成,verify-serve T5 守漂移 |
...
| Import plan | `services/import-job-service.ts` + `pure/import-plan.ts` + `import-plan-store.ts` | 导入编排器(github/folder/plan 三 spec 共用一条 5 步路径):每步产物快照落盘断点续跑、同源再导自动复用(零 LLM)、treeHash 漂移检测 + bestEffortStructure 尽力保留;课程包=github 快照文件直接分享(`import:resume`/`import:importPack`/`import:exportPack`);planId 标注包住 Steps 2-5——任何一步失败渲染层都能给"从断点重试";**全链路取消穿透**:fetchFn 收口注入 signal(README/大纲/正文/图片在飞即撕)+ httpsGet signal(树扫描可断)+ 各网络循环取消检查,外层 try/finally 保证轮询清理;savePlan 落盘审计日志(写 lookatstudy-import.log) |
...
- **Tests live in `scripts/verify-*.mjs`** (89 suites) — run via `tsx`, import real TS source.
