...
| 学习者画像 | `shared/learner-profile.ts` + `src/main/services/proposal-service.ts#update_learner_profile` | 声明侧画像（区别于 memory 推断侧）：MBTI 快捷入口展开 style 四维真源（start/interaction/feedback/pacing，16 型双语表+场景题兜底），settings `learner_profile` JSON（**未加表**），`profile:get/set` 结构化通道；注入提示词第④层（agent-engine profileBlock，独立于 node-bound 快照——无课也生效；防注入标注+合意困难条款）；AI 补画像走 `update_learner_profile` 工具 Propose→Apply（证据≥2 才提议；apply 前仲裁——提议发起后用户手编则判 stale 不覆盖） |
