...
- 飞行器机械臂(用户拍板):载具两缘新增一对程序化机械臂,接管原 bot 的手臂动作——敲键盘逐键拍打、朗读时指向高亮句、写字持笔等全部自动落机械臂(自定义纸偶与 Shimeji 都吃这条;纸偶原本弯 PNG 臂的姿势改由机械臂执行,纸偶手臂保持自然垂放)。Shimeji 的载具+机械臂只在干活时出现(打字/写字/指向朗读句/飞行),平时地面散步、坐下、爬墙时收起,值勤时自动落到平台中央。
