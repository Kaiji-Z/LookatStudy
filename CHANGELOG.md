...
### Added
- **视频链接导入(B站直连 / YouTube 等经 yt-dlp)** —— 智能 URL 框现在认视频链接:`bilibili.com`(BV/av/分P,含 b23.tv 短链)免登录直连拉 DASH 音轨(纯 JS 实现 B站 wbi 签名);YouTube/抖音等上千站点检测本机已装的 yt-dlp,**字幕优先**——有 CC/自动字幕直接出文本,零转写零模型,无字幕才下载音轨走 Whisper 转写(与音频导入同款,模型缺了自动下载、可取消);未装 yt-dlp 给出平台安装指引。视频内容照常进 AI 结构设计成课,音轨字节随快照缓存,断点续跑零二次下载。
