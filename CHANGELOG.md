...
### Added
- Shimeji 桌宠作为伴学伙伴第 7 形态(一期):设置页伴学区新增「Shimeji 桌宠」区块,+ 号卡片导入 zip——双格式通吃(日版原版 Group Finity 与 Shimeji-ee,标签/字段一一对应解析),zip 内多角色(自包含目录布局与引擎布局都认)弹出角色勾选窗,勾选后落盘为包。激活后到伴学形象选择器选 Shimeji 即可换装:帧动画渲染器 + 独立自主调度循环,动作按包内定义播放,表情联动到对应动作。贴边物理(二期):抓起=挣扎帧跟手,松手轻放原地缓冲,快扔=扔出坠落重力落地;地面会自主步行折返、坐下、躺平,并能沿边缘爬墙→爬顶→横行→坠落落地(动作按 88 归档表的窗口环境槽位接入:地面/墙/顶/面板/鼠标系均可被调度;旧格式导入的包自动按动作类型退化)。拖拽运输仍由原伴学飞行物理承担,原五形态与自定义纸偶行为零改动,随时切回。
- 飞行器机械臂(用户拍板,两次修正后定稿):载具两缘新增一对程序化机械臂,接管手臂动作——敲键盘逐键拍打、朗读时指向高亮句、写字持笔。分工:**能切分手臂的纸偶,手臂动作仍由纸偶自己的手臂完成**(rest 角补偿姿势链不变,手臂不会因机械臂存在而静止);机械臂只给切分退化到 L1 整图(无臂件)的纸偶兜底。Shimeji(精灵图无臂可动)的载具+机械臂只在干活时出现(打字/写字/指向朗读句/飞行),平时地面散步、坐下、爬墙时收起,值勤时自动落到平台中央。实测反馈三修:平台收起时机械臂一同收起(此前臂画前景时漏出门控)、爬墙入口暂时关闭(壳层无贴边物理,攀爬帧=飘着爬空气墙;归档与代码路径保留,待爬墙锚点旁路方案立项重开)、活动范围收紧到精灵半宽内(贴边时半个身子不再飘出舞台)。二次反馈定稿:载具+机械臂改为常驻(bot 本就悬浮,脚下没平台没道理),仅爬墙/爬顶时收起;「墙」对齐到可见容器边缘(输入卡/讲解面板的左右缘与顶缘,按窗口实时换算),爬墙爬顶贴着真实 UI 边走;击键字符落到平台仪表舱玻璃上(与自定义纸偶同链)。课程切换新增入口:课程地图顶部的课程名称现在可以点击——弹出全部课程的列表浮层(当前课标亮),点选即切换,不必再去导入课程页;导入页的课程列表照常可用。躺/坐等rest动作时身体自动缓移回台面中心(躺帧横向铺满帧宽,散步范围保证不了极端姿态落在台面内——实测反馈"躺下半身在平台外")。伴学设置页整合:新建入口统一到一张「+ 新建」卡——点击弹出选择 自制/Shimeji(悬停显示两者区别介绍),自制进制作向导、Shimeji 进包导入弹窗;自制的每个纸偶包和导入的每个 Shimeji 包都以形象卡混排在伴学形象区,点卡即激活上岗。(二次整合定型:形象区固定 7 张卡——5 个原形态 + Shimeji 卡 + 自制卡 + 新建卡;点 Shimeji/自制卡弹出已导入/已制作的包列表,选一个即激活持久化,包列表内可删除、可换载具主题;Shimeji 的载具主题同样可换(色点换装,manifest 持久化,机械臂与平台同色跟随,激活包实时换肤)。)二次整合定型
