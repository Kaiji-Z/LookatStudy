...
- 飞行器机械臂(用户拍板,两次修正后定稿):载具两缘新增一对程序化机械臂,接管手臂动作——敲键盘逐键拍打、朗读时指向高亮句、写字持笔。分工:**能切分手臂的纸偶,手臂动作仍由纸偶自己的手臂完成**(rest 角补偿姿势链不变,手臂不会因机械臂存在而静止);机械臂只给切分退化到 L1 整图(无臂件)的纸偶兜底。Shimeji(精灵图无臂可动)的载具+机械臂只在干活时出现(打字/写字/指向朗读句/飞行),平时地面散步、坐下、爬墙时收起,值勤时自动落到平台中央。
