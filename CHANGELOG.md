...
### Added
- **伴学 v6:朗读句级跟随(karaoke)** —— 整课朗读时讲解区**高亮正在读的那句话**(accent 蓝轻底,句子滚出视野才平滑居中跟随,不逐句乱跳),伴学伙伴**飞到当前句旁边抬起手臂指住这行字**,读一句挪一句(右侧站不下自动换到左侧、指向随侧翻转);朗读的**🔊 按钮固定悬浮在讲解视口右上角**,正文滚到哪都能一键停,不用滚回顶部。句子定位与合成侧同源(`shared/speech-text` 纯函数切句),高亮跟随的是**播放序**而非合成到达序(合成快于收听,到达序会超前 1-2 句)。
