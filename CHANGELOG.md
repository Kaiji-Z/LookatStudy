...
- **思维导图渲染不全(两处)** —— ①初次渲染/切课时 fit 在节点布局沉降前执行会按空或旧边界适配,偶发内容出界:改为双 rAF 后 fit 加 300ms 兜底补一次;②展开被折叠的节点后内容边界变大而 autoFit 是关闭的,展开内容被裁切:节点点击后自动重适配。**结构保真重设计**:markmap 只认标题与列表,裸段落会被当作上一列表项的懒延续吞掉——没有小节标题的导入课时(切片正文全是段落)整篇缩成一个节点,脑图完全失去结构。现在段落降为节点:每段取首句(粗体引导段取粗体内容)做文案、40 字符截断,悬在最近标题之下成为结构的一部分;表格与分隔线跳过。段落→节点的转换是纯函数(paragraphTopic),无小节标题的课时从此也能得到一张真实反映内容的概览图。
