...
### Fixed
- **拖选扩展不再被浮钮卡断** —— 拖选后想再多选几个字时,指针扫过「提问这段 / ✏️ 加笔记」浮钮会让选区漂移(浏览器在按钮上解析不出文字插入点):现在拖选手势进行期间浮钮可见但穿透(pointer-events:none),松开即恢复可点——保留拖选途中浮钮即现的即时感,向哪边拖都撞不到;按下点在浮钮自身(要点按钮)不误判。浮钮定位维持右侧优先(避开手机原生选择菜单),并修正多行拖选的落点:跟随**最后一个字**(末行行盒),不再悬在整块选区外接框的右缘中部。讲解区与对话消息两处同款。
