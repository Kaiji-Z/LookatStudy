...
- **T3 双行标题栏(切换栏满宽化)** —— 窄屏切换栏从右上角三个小按钮改为标题栏**两行布局**:下行 = 满宽三等分切换栏(地图/对话/笔记本,图标+文字,h-11 符合手机底部导航习惯;两行总高 106px),上行 = 原标题栏内容自适应(标题收 text-label、能量只留闪电+数字)。宽屏(T1/T2)标题栏不变。E2E 已验证 390px 下切换栏满宽、三按钮带中文标签。
