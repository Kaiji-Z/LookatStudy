...
- 手机端考试伴学遮题（2026-09-19 用户反馈"是否应该跟着时间进度条移动"）：桌面陪考位=计时条右侧空边距，窄屏没有这块边距——x 被钳到屏幕右缘、y 落进题干区压住题目。窄屏改**骑总进度条**：考试顶栏是固定条不随题干滚动，锚点钉在进度条填充前沿（答完一题滑一截，兑现"跟着进度条移动"），横向钳进"第N/M题…计时"之间的空带，永不进题干区；宽屏行为不变。ExamView 进度条加 `exam-progress-bar`/`exam-progress-fill` testid；ui-test 新增 420px 窄窗骑条守卫（onBar + 不与题干矩形相交），探针退场轮询验证真离开（离开确认偶发一击不中→包重跑间歇红的根因）

## [0.38.1] - 2026-09-18

### Fixed

- 概念图间距放宽（用户实测"太紧凑，连线标签把箭头和连线都遮住"）：层间距 56→80、同层 34→46、边-节点 26→36、组内边距 14/12→24/20。**根因二连**：①间距参数本身偏小；②elkjs 的 layoutOptions 不向复合子图级联（`INCLUDE_CHILDREN` 也不）——组节点不重复挂 spacing 时组内行距跌回默认 20px，截图里"组容器内部挤成一团"的真凶；现组节点与根同源共享一份 `SPACING_OPTIONS`（verify T3 钉"不级联陷阱"守卫）。实测模拟夹具：层间距统一 ≥80px（原最低 20px）、最差标签距边端点 10→40px（短边箭头不再被胶囊压住）、画布 321×549→377×737。标签仍贴线白晕（拍板：不做线旁摆放/隐藏）
...
- 伴学伙伴胸屏覆盖层脱锚：连对金环/击键字符面板/听写声纹条从 SVG 根坐标迁入形态 `cp-bot` 组内渲染（`FormArtProps.chest` 槽注入，七形态全接线）——此前待机浮沉（cp-bob）、姿势旋转、逐键压弹时覆盖层跟不上身体，金环和字符"浮"在胸屏外
- 伴学"抬头等待"表情与真听写拆分：打字暂停相位 1（停手 1.2~6s）改用独立 `waiting` 表情——此前借用 listening，而 window 级键击/点击都会触发暂停相位，导致胸屏声纹条/头屏声波弧在闲置时"时不时亮起"；听写专属视觉件现在只响应真实听写信号
- 停听写清零麦克风平滑包络（`companionSetListening(false)`）：`micSmoothed` 此前冻结在停录瞬间音量，之后任何 listening 表情都会让头屏声波弧以"幽灵音量"常亮

## [0.36.0] - 2026-09-15

### Added

- 开屏/资料窗 UI 质检轮（impeccable 双通道评审后单批修复）：兴趣输入框不再边打边吞分隔符（显示与解析分离，失焦归一）；向导三问步收敛为单卡流（外层场景卡让位子卡+诚实跳过链接"先跳过这三问"）；三问子步进 1/3→3/3（外层步进点不再冻结）；course_pick 的"完成"按钮从 no-op 死按钮改为真实收尾；个人资料弹窗支持 Esc/点背景关闭、保存 Toast 反馈、记忆删除走 ConfirmCard 确认；单选组（动机/MBTI/场景题/试玩题）补 radiogroup/radio ARIA 语义；全量触控目标补 padding（此前 20-24px 低于 44px 地板）；试玩题庆祝粒子只在实际更站得住的一侧触发（修"差一点!"配 correct 庆祝的动效矛盾）、未答前两选项同为中性色；MBTI 翻卡移到网格下方+入场动画（reveal-enter，reduced-motion 自动降级）；场景题↔16 宫格双向切换；历史状态 emoji 图标换 lucide；en 语境的中文全角括号/逗号清理；`.btn-3d` 统一禁用态；streak 火焰改 review 橙（原误用 error 红）；画像摘要补兴趣行
...
- 画像注入与导师人设（soul）的优先序明写：【风格适配】条款末尾新增"若风格与当前选定的导师人设冲突，以人设为准——当场的显式选择压过静态画像"（zh/en 同构）——此前优先序只靠"背景数据非指令"的语气隐性成立，引导派 × I 型组合下模型会在提问与讲述间摇摆
...
### Added
- Shimeji 桌宠作为伴学伙伴第 7 形态:导入任意 Shimeji zip(双格式通吃——日版原版 Group Finity 与 Shimeji-ee,标签/字段一一对应解析;zip 内多角色弹出勾选窗,自包含目录布局与引擎布局都认),激活后到伴学形象区选 Shimeji 即可换装:帧动画渲染器 + 独立自主调度循环,动作按包内定义播放,表情联动到对应动作;旧格式导入的包自动按动作类型退化。
- Shimeji 贴边物理:抓起=挣扎帧跟手,松手轻放原地缓冲,快扔=扔出坠落重力落地;地面会自主步行折返、坐下、躺平,躺/坐时身体自动缓移回台面中心(极端姿态也整个落在台面内)。爬墙入口暂时关闭——壳层无贴边物理,攀爬帧=飘着爬空气墙;归档与代码路径保留,待爬墙锚点旁路方案立项重开。拖拽运输仍由原伴学飞行物理承担,原五形态与自定义纸偶行为零改动。
- 飞行器机械臂:载具两缘新增一对程序化机械臂,接管手臂动作——敲键盘逐键拍打、朗读时指向高亮句、写字持笔。分工:**能切分手臂的纸偶,手臂动作仍由纸偶自己的手臂完成**;机械臂只给切分退化到 L1 整图(无臂件)的纸偶兜底。Shimeji(精灵图无臂可动)的载具+机械臂只在干活时浮现(打字/写字/指向朗读句/飞行),平时散步、坐躺时收起。载具五款主题色点换装(manifest 持久化,机械臂与平台同色跟随,激活包实时换肤),击键字符落到平台仪表舱玻璃上。
- 伴学设置页整合:形象区固定 7 张卡——5 个原形态 + Shimeji 卡 + 自制卡 + 「+ 新建」卡。新建弹出选择 自制/Shimeji(悬停显示两者区别介绍);点 Shimeji/自制卡弹出已导入/已制作的包列表,选一个即激活并持久化,列表内可删除、可换载具主题。
- 课程切换新增入口:课程地图顶部的课程名称现在可以点击——弹出全部课程的列表浮层(当前课标亮),点选即切换,不必再去导入课程页;导入页的课程列表照常可用。

### Fixed
- 复习完成后地图上的复习徽章不消失:复习自评(`srs:record`)只发 `state:changed "xp"` 事件、不发 mastery(自评不动 BKT),而渲染层只在 mastery/全量刷新时拉取到期集合——现在 xp/streak 事件也补刷到期集合,自评一次徽章即时缩一次;quiz/exercise 答题同样受益。

### Internal
- ui-test / shots 运行模式的文件目录改指系统临时目录(此前与 dev 共享真实 userData,清理类断言曾误删用户导入的 Shimeji 包)。
...
### Added
- 语言学习课程支持(双轴语言模型,issue #15):导入时 LLM 判断课程是否在教一门语言本身(词汇/语法/课文/水平分级特征;判断不准倾向否),被教语言落库到 `courses.language_target`。命中后教学切到"目标语言模式"——讲解、指令、反馈仍用你的界面语言,但课文引用、例句、题目中的语言素材保持目标语言原文出题(此前中文界面学英语,题目和原文会被整体翻译成中文,语言学习失效);对话导师追加语言教学姿态(学习者用目标语言表达时先肯定再纠错、新词注释不整段翻译、随掌握度提高目标语言占比)。英语/日语/韩语/中文等任何语言组合通用(教材书写语言与被教语言独立判断),章节考试与课后练习同一双轴链。已有课程不受影响,重新导入即可生效。
...
- **多 bot 管理**:每保存一个 bot,形象栏持久化一张卡片(缩略图+自定义名字);点卡片切换使用,右上 × 删除(确认弹窗);重导同一张图幂等不重复。
- **悬浮载具**:自定义 bot 站在一架程序化机械飞行平台上(金属三层面+仪表舱+三喷口火焰,飞行时加力)。五款主题对应内置五形态设计语言(小焰熔岩橙/霜绒冰蓝/苔芽/星尘紫夜+星点/墨墨漆黑+朱砂),制作向导里选择,形象栏卡片左上角色点随时可换(含银灰中性款),切换即时生效。
- **朗读指向自动校角**:自选立绘的手臂角度千差万别,导入时自动量测"肩→手尖"方向,朗读跟句指向按目标角−静息角求差修正,A-pose 素材不再指偏。

### Fixed
- 自定义 bot 卡片缩略图与身体定位偏心(壳层航灯悬空绿点+布局坐标混用画布宽高两处根因);部件白描边在深色曲线处凹凸不平(抗锯齿逐列抖动被膨胀放大,现膨胀前先平滑)。

## [0.30.0] - 2026-09-09

### Added
- 设置页「数据」区:从 dsh 插件迁移学习进度——桌面版自动探测本机 `~/.dsh/lookatstudy-plugin/state.json` 一键导入,任何平台(含 serve/手机浏览器)可上传 state.json 文件导入。**全量迁移**:课程(含正文/摘要/双语翻译)、进度、per-KC 掌握度、SM-2 复习、考试星数、康奈尔笔记(画线+注释,quote 优先当画线文本)、黑板产物(概念图/对比表/流程图/代码讲解;guess 类型无画布对应,过滤并计数披露)、学习者记忆(全局风格+课级模式+课级 gap 三槽)、卡点日志、XP 增量合并、streak 取较大;map 模式(同标题同结构既有课)只写进度与笔记/卡点/记忆,不碰既有正文/摘要/翻译。导入前自动备份数据库,重复导入安全,兼容插件全部历史 state 版本(v1/v2)。与 AI 的对话历史不迁移(源数据在 dsh 侧会话存储,不在 state.json)。CLI 脚本(`scripts/import-dsh-progress.mjs`)保留给无 GUI 场景(核心进度)。
...
### Added
- 三栏布局拖拽调宽:课程地图 / 导师对话两栏的栏间边界可拖拽调整宽度(右栏笔记吃剩余空间),宽度经 settings 持久化,重启/重载后保持;双击手柄恢复响应式默认。钳制视口感知(可调区间 左栏 240–480px / 对话栏 480–1100px,且恒不把笔记栏顶出屏幕——窗口后来变小会把旧定制宽自动压回预算内)。触屏手柄命中区 44px,细指针收窄 12px;拖拽期间宽度直写不经过 React 重渲染,左栏物理岛按 100ms 节流跟随重建。([#14](https://github.com/Kaiji-Z/LookatStudy/issues/14))
...
### Changed
- 伴学视觉打磨(实机逐状态截图审查驱动):星尘(astro)形态外轮廓改亮缘描边(家族"暗描边"语法对暗色身体的反转延伸)——旧深紫描边与左栏夜空几乎同色,地图上整体隐身;纱帘待机分档,左栏(锁定深色)下 0.3→0.55 透明度+轻模糊让待机点重新可辨,内容栏浅底维持原样;左栏待机空地挑选(`pickPerchBase`)净空计入栏缘,空旷地图不再选中贴分界线的边缘列(身体压线的"挤墙角"观感);庆祝召唤栖位(`celebrationPerch`)落点外推 +0.4×size,身体左缘不再骑在 origin 所在行(答对题时压住选项/题干文字)。

## [0.27.0] - 2026-08-31

### Fixed
- **AI 导师手写假工具标记导致掌握提议无卡片** —— 真实事故:默认思考档下模型不真正调用 `mark_mastered` 工具,而是在正文里手写「[工具调用已执行] mark_mastered → …」假标记(模仿系统注入历史的标记格式),学习者界面上没有确认卡片、无按钮可点。base prompt 双修:①工具清单补 `record_answer`/`mark_mastered` 的使用指引(此前只列 6 个展示型工具,模型对"怎么发起提议"的唯一参照就是历史标记格式);②新增反伪造条款——标记只能由系统注入历史,正文手写不产生任何界面产物,发起提议的唯一途径是真正的工具调用。verify-agent-locale 新增"手写标记事故锁"断言(工具清单点名 + 反伪造关键句不被误删)。

### Changed
- **base prompt 三级分层重构(红线→行为→偏好)** —— 指令密度随事故条款只增不减会稀释模型遵守率(手写标记事故的诱因之一),基座提示词从单体补丁堆重组为三级:【安全红线·最高优先级】(防幻觉+工具调用真实性,显式声明冲突时以本段为准)→【教学行为】(模糊提问+教学工具使用)→【回答排版·偏好级】(自声明次级地位)。全部条款原句保留,仅重组结构与优先级标记;verify-agent-locale 同步新基线并新增"三级分层结构锁"(分块存在性+顺序断言,closed-loop 破坏验证通过)。层间协议沉淀 `dev-docs/PROMPT-LAYERS.md`(四层组装职责边界+层间契约:策略层指挥的行为必须在工具层有对应指引)。
...
### Changed
- **抽屉按手机最小宽适配** —— 设置/复习两个抽屉的宽度本来就是全随视口(手机端全宽,桌面端只是上限帽),这轮补齐的是**内部布局的手机端保障**:设置页伴学形态选择从固定五列网格改为自动换行(420px 视口下五列每格仅约 57px 必然挤压),连接测试结果行允许换行(长错误文案不再撑破)。新增真 GUI 断言:窗口压到 420px(手机浏览器视口,不受桌面窗口 560px 最小宽保护)分别开两个抽屉,断言零水平溢出;源级守卫锁抽屉宽度必须视口相对。

### Added
- **伴学 v13:标题栏也是居住空间** —— 左栏不在场时(窄屏 T3 未选左栏/未选课程/T2 收起左栏)的家从"左缘停靠"升级为标题栏栖息地:伴学缩小到 44px 在标题栏横向缓游(约 40 秒一个来回),与栏内按钮/输入框重叠时自动半透明+点击穿透(不遮按钮),可以随手拖到标题栏任意位置;拖出标题栏松手则恢复该栏体型,留在放置处 ±22px 小范围徘徊(不回家、不满屏游,用户觉得遮挡自然会再挪),左栏回场时自动恢复常规回家。左缘停靠仅保留朗读跟句悬空时的一处兜底。
- **召唤名单再扩展:流式思考也飞来** —— AI 流式回答期间伴学飞到输入卡上空(与聚焦输入框同一位置)做托腮思考,流式结束经防抖回该去的地方。表情/姿势复用既有 thinking 链,零新增资产。

### Fixed
- **加笔记后的多余停留** —— 划线加笔记时伴学飞到新画线旁做记录动作(此为既有行为),但动作结束后要先在讲解面板右上角停满 3.5 秒防抖窗才回家。现在笔记动作结束直接回家(写作动作的 2.2 秒本身就是驻留);朗读结束仍走防抖(别刚停读就抖走)。另:点击右栏"笔记"标签页不再召唤伴学——切标签不是记笔记。

## [0.25.0] - 2026-08-23
### Added
- **召唤名单扩展:听写也飞来,答对庆祝/解锁也飞来** —— v12 召回制的召唤面补齐:①听写进入语音模式即算中栏信号(按住说话时伴学飞来输入卡上空陪伴),此前只有键盘聚焦召唤;②答题答对(题卡/复习自评)与节点解锁触发庆祝召唤——伴学飞到事件源旁(题卡右上/解锁球旁)庆祝约 3 秒再回落回家,事件坐标随 celebrate 总线下发(`origin`),答错/其他高光不召唤(原地特效不扰动)。锚点健壮性两守卫:解锁球的 rect 必须真在视口内才携带(进度数据二次加载时球可能还虚拟化在屏外,负坐标锚点会让伴学朝屏外冲刺,ui-test 取证修复);庆祝落点 `elementFromPoint` 取不到宿主元素时(屏外/未布局)不召唤,绝不兜底猜栏。考试答题中不抢陪考位。

### Fixed
- **ui-test 隐藏窗口 rAF 停摆(伴学动画断言假红)** —— ui-test 的无头窗口从未 `show`,拿不到合成器 begin-frame:rAF 先被降为约 1 秒一帧、约 16 秒后完全停摆,伴学冻死在游走半路,exam-perch 等位置断言读到永不更新的 transform(假红;此前的 `backgroundThrottling:false` 只救定时器救不了合成器)。修复:非截图模式下透明 show(合成器全速,视觉仍无头);取证用每帧快照(帧序号/时间戳/可见性)锁定"帧间隔严格 1016ms 后全停"的铁证。真 GUI 53 项全绿(清理插桩后复跑确认)。

### Changed
- **伴学伙伴 v12 召回制:游走只在左栏,中/右栏有事才到,办完回家** —— 用户实测反馈:伴学闲时栖在中栏/右栏会遮挡阅读。新契约把"陪伴优先"翻转为"召唤制":自由游走只发生在左栏地图(原跨栏游走退役);中/右栏只在有事时到场——输入框聚焦(打字陪伴)、整课朗读(跟句指向)、划线记笔记等信号触发飞来,信号结束经 3.5s 防抖后飞回左栏。左栏不在场(手机单栏/左栏收起/空态)的家改为**左缘停靠**(半身探出屏幕边缘等事),替代原"整窗游走";手机端由此同一套语义解决:有事从左边飞入,办完飞回左边。朗读但跟句锚未就绪时退到讲解面板右上肩停靠,不再游进文章正文;跟句悬空兜底同款。**拖拽落点修复**:松手后 rail 物理刚体同步到落点——旧实现滑翔目标是拖拽前的旧刚体坐标,观感是"回原点";现在从落点连续飞往职责锚或回家,全程限速滑翔不瞬移,新增回家巡航档(比赶场慢、比闲逛快,有来有往)。真 GUI ui-test 全绿(空态左缘停靠坐标实测吻合、聚焦飞中栏、朗读去右栏、T3 切栏单生物);破坏闭环四向见提交记录。
...
### Fixed
- **导入管线四路真实样本健壮化(PDF/网页/字幕/PPTX,采样→量化→修→双锁定)** —— 与电子书同轮的方法论推广到其余四条解析路径,每条:真实多样本语料量化特征 → 修高置信度规则问题 → live 语料套件(gitignored 缓存,CI 外可重跑)+ verify 合成 fixture 双锁定 → 破坏验证闭环。①**PDF**:`normalizeRadicals`——pdf-inspector 对中文 PDF 输出康熙部首区字形(U+2F00-2FDF,"手"变"⼿",画线/朗读/检索全断),双引擎出口统一逐字符 NFKC 归一;6 真样本语料核查部首零残留,扫描版诚实空串。②**网页**:`stripTailNavigation` 尾部站点模板指纹清理(搜狐"返回搜狐,查看更多"行/阿里云侧栏三行/行内导航后缀/CSDN 裸图路径行)——原则:规则只删跨文章稳定的机器生成模板,作者自己写的推广段是正文(实测教训:按"欢迎关注公众号"做规则把 CSDN 作者亲笔推广段误删过),尾部语义边界交 Step4 LLM 判断(prompt 新增"网页来源的正文边界");知乎 403 诚实报错。③**字幕**:`parseSubtitleToText` 三修——YouTube 自动字幕滚动窗重复(cue N+1 首行复述 cue N 尾行,旧相邻去重挡不住,成文每句出现两遍)近窗去重修;中文字幕逐句 join 空格污染(真语料实测 78+262 处 "汉字 空格 汉字")CJK 感知接行修;yt-dlp 落多语言字幕取件 readdir 字母序让 en 永远压过 zh-Hans 的优先级 bug 修;附带 HTML 实体解码/[Music] 封闭集整行标记/行首 >> 换说话人标记。④**PPTX**:officeparser 的 table 节点此前被整层忽略、表格文字全军覆没(真实课件表格极常见)——`tableToMarkdown` 转 GFM markdown 表进正文,竖线转义防破表,占位符未填的全空表整张跳过;12 个真 PowerPoint fixtures(含断关系项/零页/无元数据病理件)零抛错,图表页文字不提取仍为已知局限。新增 4 个语料核查套件 live-test-{pdf,web,subtitle,pptx}-corpus.mjs + verify 三处扩断言(verify-pdf-text T4-T7 / verify-html-article T6-T11 / verify-video-import T4b-T4d / verify-pptx-parser T4-T5);各档破坏闭环见提交记录。
...
- **种子课程内容全量核对** —— 六课落在产品现状后面的过时表述按当前设计重写(中英双语同步)。
...
### Changed
- **入口包瘦身(内部优化)** —— 渲染层主束 `main-*.js` gzip 397KB→276KB(-30%):非首屏重组件(设置抽屉/考试视图/命令面板/课程搜索/脑图/AI 产物渲染器/黑板画布/复习自评卡/语音面板)改 React.lazy 按需加载;数学插件集(katex+remark-math+rehype-katex)收口 `lib/math-plugins.ts`,只对含公式内容动态加载,插件序 raw→sanitize→katex 不变。启动不再为"暂时用不到的界面"买单,手机 web 模式同步受益。
...
### Added
- **伴学竖线眼** —— 五形态全表情统一为 EVE 式竖圆棒眼(旧圆角矩形瞳孔眼退役),情绪改由棒的长短/粗细/浓度表达:常态双竖棒、惊讶拉长收细、思考左短右长、聆听微短、睡觉两粒短淡点。竖线的两种变形:开心/欢呼/得意=弯棒上拱弧(^_^ 笑眼),鼓脸余怒=旋横 90°(-_- 无语眼)。视线跟随鼠标升级为整眼平移(棒渲染进瞳孔组,跟随幅度放大到 5/3px 可感知);眨眼压扁竖棒成短点,与口型/打字动画链零冲突。verify T15 守竖线词汇+变形串+视线幅度。

### Fixed
- **拖选浮钮:松手才显示 + 分端定位/尺寸** —— 「提问这段 / ✏️ 加笔记」按钮改为选区**选定后**才出现:拖选/调整选区途中一律隐藏(指针路线再也不会被按钮卡断),桌面松开立即落位,手机拖选柄松开按 600ms 稳定窗口落位(拖柄途中的短暂停顿不弹)。定位分端:桌面在选区最后一个字右侧(多行选区跟随末行行盒,不再悬在整块选区外接框右缘中部),手机锚选区**下方**——同时避开拖选手柄(原右侧锚点正贴着手柄,抓柄会误触)与上方原生选择菜单。按钮组固定单行永不换行;触屏端放大到 44px 手指命中底线,桌面维持紧凑。讲解区与对话消息两处同款。
...
- **mermaid 流程图 ELK 布局** —— AI 画图(draw_diagram)产出的流程图换 ELK 布局引擎(正交路由,复杂流程的边交叉显著少于原 dagre):渲染层把 `flowchart`/`graph` 前缀改写为 `flowchart-elk` 再喂给 mermaid,**LLM 出题提示词与语法修复回路零改动**(继续产出/修复原版语法);sequenceDiagram/stateDiagram-v2 等其他图类型不受影响。代价说明:官方布局包 `@mermaid-js/layout-elk` 发布件内联自带 elkjs 0.9.3(644KB gzip 懒 chunk,与概念图的 elkjs 0.12 无法去重——已知情接受双份),只在首次看流程图时加载一次,主束零污染(新增 verify-build-manifest 断言:主束不得内联 elkjs/shiki/mermaid,elk 懒 chunk 唯一)。verify-mermaid-elk(改写纯函数 13 例+真 parse 集成+接线守卫)+ 破坏验证 + ui-test 真渲染断言(ELK 生效判据=mermaid 的 dagre 兜底 warn 缺席)。
...
- **考试时伴学静栖计时区** —— 开考后伴学钉在计时条旁陪考:不漫游、不进物理世界,庆祝动作(出拳/跳跃)、点球互动、吹哨召唤、击键压缩全部静默,只留 800ms 的轻表情;生成题目/就绪页没有计时条时照常活动(绝不隐匿),交卷即恢复。verify-companion T28 + ui-test 落位断言。
- **每题限时动态放宽** —— 章节考试 60/90 秒二档退役,改为按题干内容实时计算:45 秒基础 + 中文字数÷5 + 英文词数÷3 + 选项数×8,含代码块或 LaTeX 公式再各加 25 秒,上下限 60~300 秒。短题不拖沓,长题干/公式题给足(300 字长题约 137 秒,800 字带代码约 270 秒);限时只在答题时计算,历史成绩回顾零迁移。verify-exam T3 重写。
...
### Added
- **吹哨召唤伴学** —— 点击左栏地图的空白处(球/按钮/路牌不算),伴学飞到点击处右上悬停,挥手应答"来啦"(一声轻快的提示音)。拖动/滚动手势的松手不误触(位移 >6px 判定为手势);吹哨会把它从任意栏叫回左栏,悬停约 3 秒后回归日常。哨点自动避开标题栏禁入带(见下)。verify-companion T26 守链。
- **标题栏禁入带(伴学永不进标题栏)** —— 三层守卫:①左栏物理世界接**硬天花板**(撞线反弹,风/球/扔掷都顶不上去);②吹哨哨点钳在带下(点在 tab/课名条上不会把它叫进带里);③渲染收尾**视觉钳带**兜底全部分支(整窗游走/跟句首行锚点/拖拽进带都压回带下)。带底 = 应用标题栏(T1/T2 与 T3 窄版)与左栏 tab/课名悬浮条取深者。verify-companion T27(物理反弹/接线/哨点钳制)。
- **键盘反馈六件套(伴学胸屏=信号面板)** —— ①胸屏新增**击键脉冲波形**:7 条确定性伪随机信号条随每次击键扫过能量核(与听写声波同族视觉,机器人在"处理信号"而非"显示文本");②左右臂按 **QWERTY 真实键位**按压(左半区=左臂,未知键沿用交替)——像"看着你的键盘";③每次击键整机微压缩(节奏呼吸),3 秒内连打 6 键进入**专注态**(屏内思考眉);④打字暂停有表情:停 1.2~6 秒抬头等待,6~15 秒若有所思,之后回常态;⑤**回车=交接仪式**(小跳+胸屏 → 闪发,"我打完了该你了"),连退 3 格退格=屏角冒汗(注意到你在返工,不打扰);⑥**中文输入法提交的汉字上屏**(compositionend,关闭 v10 的 IME 缺口,中英文打字反馈对齐)。reduced-motion 全动画关闭。verify-companion T25(键位分区/波形确定性/reducer 分型/暂停相位/接线)。
- **点课程球伴学互动** —— 点击(或搜索跳转)课程球,伴学飞到球的右肩位**指住它**并轻顶一下球(庆祝性拨弄,锁定球不动);T3 手机端点球自动切中栏、左栏不在场时,伴学在当前栏**朝左注目礼**应答(左栏方位)。三条跳转路径(物理球点击/键盘球/搜索跳转)都触发;坐标缺失时由物理岛按 nodeId 定位。verify-companion T24 守接线。
...
### Fixed
- **加速/变向喷焰从未可见** —— 焰粒子一直画在距旋转原点仅 ~15-23px 处,整段被身体轮廓盖住(起飞只是靠增大半径+辉光勉强探头,速度焰从未露出过)。粒子重画到身形半径外做**定向尾焰**,焰长随速度伸缩(静止缩半贴身,全速展开)——现在滑翔/加速真的看得到推进器。
- **伴学在朗读后可能永久隐身** —— 朗读中把承载高亮的面板卸载(切课/切线程/T3 换栏)会留下指向已卸载 DOM 的悬空朗读 Range:伴学跟句分支拿悬空锚点算不出落点,而该分支此前没有兜底,直接 `opacity 0` 从此一直隐身("常驻绝不隐匿"被旁路,触发动作无感知)。两层修复:①跟句分支悬空时就近游弋兜底(与记笔记分支 v11.1 同款守卫,绝不隐身);②两面板的卸载清理改按**渲染期快照**清全局高亮——旧写法在 cleanup 里读 ref,而 React 卸载时先把 ref 置空再跑 passive cleanup,条件永远为假,守卫自废。verify-companion 新增 T23(含"卸载清理不得读 ref"的回归禁令)。临时自救:再点一次 🔊 开始随后停止,高亮重建后伴学即回来。
...
- **中栏/右栏投影重做** —— 脚下的小椭圆地影删除,改为 **drop-shadow 整身投影**:跟随机器人剪影(抬臂/旋转/压缩时影子大形自动跟着变),模糊抽象、偏移右下对应左上光源,纯合成器搬运零逐帧运算——"跃然纸上"。左栏在天空,继续无影。
- **手机端对话工具栏布局修复** —— 上一版 40px 触控命中把带文字的按钮撑大导致换行错乱。现在粗指针设备上工具栏改**图标按钮**:文字标签与箭头隐藏、模型钮补图标,🎤/📎/思考/上下文/模型 五钮单行放得下;输入框本体与桌面端零变化。

### Changed
- **伴学面部精细化:表情全部收进头部屏幕 + 嘴型机器人化** —— 思考眉此前画在屏幕外的盔额上,声波/汗滴/蒸汽/zzz 也散在头盔外沿;现在全部重定位进屏幕内,且五形态的脸+挂件整体加**屏幕剪裁**(构造上不可能再越出屏框),毕业金冠作为头顶实体徽章保留在剪裁外。嘴型去掉"人类器官":粉色舌头/珊瑚唇/叶舌/朱砂舌/紫唇全部换成**屏光件**——舌头=发光音素条(各形态屏色),牙齿=分段显示刻度,咬唇=快门齿条压住光条;口腔整体上移收进屏幕(旧版嘴部越过屏幕底边)。
- **手机端安装/升级统一同一条下载链** —— npmmirror 主源(+滞后守卫)→ **npm 官方源** → GitHub 直连 → gh 代理。`update.sh` 升级此前只走 GitHub 直连+代理(装的时候优先镜像、升级反而不试镜像);现在升级脚本由安装器生成、函数体逐字同源,且镜像滞后/未命中时先落权威 npm 官方源再碰 GitHub 链(国内少撞代理)。安装器的便携包与语音段同样补上官方源这一跳。
- **加速喷焰(推进质感)** —— 喷焰不再只绑定飞行姿势,改为**速度驱动**:移动速度点亮焰的亮度,移动方向决定焰的朝向(尾部指向运动反方向),静止即熄——移动起来有推进器的机器感,不再是幽灵漂浮。
- **手臂图层移到头部之上**(全五形态) —— 抬臂指读/挥手时手臂不再被头盔遮挡。
...
### Fixed
- **伴学跟读不再压住高亮句(整句全部行盒避让 + 四向指向)** —— 按拍板规则"生物不压正在高亮的句子,压未高亮文字可接受"重做跟读锚点:障碍物=高亮句**全部行盒**(多行句每行都算——旧逻辑只让开末行,生物上半身正压在前面几行满宽行上);候选序=整句右侧→整句左侧→**整句正下方**(多行满宽句的主位,只压下一句未高亮文字)→**整句正上方**(句贴面板底时)→全部撞句(极端窄屏)时最小遮挡位+**半透明**让出可读性。**手臂指向随方位**:句右侧指左/左侧指右(原有),句下方**抬臂指上**、句上方**外展下指**(新姿势 pointu/pointd)。像素级实测:生物与高亮行零重叠(下方 50px 净空),短句仍保留行尾右侧贴读位。
- **朗读高亮改为合成侧权威原文直挂(彻底治不同步)** —— 此前显示侧从课文**复算**一份句表再按播放序号取句,两侧句表只要有一丝分叉(净化差异/翻译切换/切段参数漂移/某类句子匹配失败)高亮就和声音错位;现在主进程每个音频块随事件**携带它朗读的原文**(`ttsAudio.sentence`),渲染层直接高亮这段文字——**念什么高亮什么,构造上不可能错句**。超长行被强制断句撕开的块按"已播前缀"并回同一视觉句,高亮随音频逐块生长。插桩实测每句高亮都在音频起播后 ~150ms 内切换。(v11.2 起换行与 emoji 本就是断句位,120 字上限只在一行内无任何标点时才触发。)
- **朗读高亮"慢一句"(emoji/符号短句冻结高亮)** —— v11.2 把换行与 emoji 定为句界后,列表里的 emoji 引导(📚/🗺️ 等)成了独立短句;而高亮匹配的规范化只保留文字字符,emoji 句规范化后是空串 → 匹配永远失败 → 高亮冻结在上一句、音频已往下读(列表/emoji 密集的课程全程"高亮慢一句")。现在规范化为空的句子回退**原文逐字查找**(emoji 句/ZWJ 连写/VS16 序列/省略号句在 DOM 里原样渲染,verbatim 命中);插桩实测每句高亮都在音频起播后 ~80ms 内切换。
- **伴学:加笔记后 bot 消失 / 朗读高亮熄火** —— 划线提交会触发重渲染换掉画线 DOM 节点:伴学"记笔记"锚点改为**文本自愈**(节点被换后按画线文本重新找回,且只有画线真挂在讲解区时才钉过去);karaoke 句匹配改用**含画线标记的文本模型**(旧逻辑跳过 mark 节点,句子含画线文字时匹配失败,加完笔记高亮直接不亮)。
- **伴学:跨栏回左栏不再瞬移** —— 滑翔目标包住物理刚体坐标(旧:先写 DOM 位置、物理引擎下一帧拉回,视觉=闪现一下)。
- **讲解区拖选即现「提问/加笔记」** —— 选区检测从 250ms 尾随防抖改为 80ms 节流,按住拖选途中按钮就出现,不用松开鼠标(250ms 隐藏延迟保留)。
- **朗读高亮不再吞整段** —— 无标点的段落/列表此前会被显示层并组成一整个"句组",高亮直接铺满到结尾;现在**换行与 emoji 都是句界**(换行整行成句;emoji 序列含 ZWJ 连写/VS16/肤色修饰整体成句;Windows CRLF 归一),显示组另有 160 字符长度上限,karaoke 高亮粒度永远可控。
- **朗读与高亮永不叉句** —— 合成侧(tts-service)与显示侧(讲解区/对话消息 karaoke)统一走 `speechSentencesOf` 单一入口拿**同一份句表**,播放序号两侧 1:1 对应(源级测试守卫);此前两侧各自内联切句,任何一侧参数漂移就会出现"高亮的字和正在念的字不是同一句"。
- **手机端对话工具栏触屏密度** —— 手机 web(Termux/Custom Tab)上底部工具栏的 caption 级按钮(🎤 听写 / 📎 附件 / 思考强度 / 上下文用量 / 模型切换)实高只有 ~28px,低于 44px 手指命中下限。粗指针设备(`pointer: coarse`)统一抬到 ≥40px 命中(图标随之放大、发送/停止圆钮 36→44px、附件移除小 X 用伪元素外扩命中不动布局);桌面鼠标/触控笔(fine 指针)零变化。
- **LAN 麦克风报错不再误导** —— 经 LAN IP(`http://192.168.x.x`)访问时点麦克风,旧文案说「请检查系统权限」,真因是浏览器不给非安全上下文暴露 getUserMedia。现在检测 `window.isSecureContext` 区分真因:不安全上下文提示改用引导器打开或配置 serve TLS,权限场景才提权限。
- **手机 web 不再出现桌宠开关** —— 桌宠是 Electron 透明窗,web 运行时(手机/浏览器)没有这个窗:设置页入口隐藏,且即使设置值为开,主窗伴学也不隐身(修「手机上 bot 凭空消失」的隐患)。

### Added
- **伴学 v11:形态平等化 + 目的性游走 + 记忆闭环 + 桌宠模式** —— **四老形态真辅音嘴型**:霜绒/苔芽/星尘/墨韵补齐 SS 齿擦(冰槽白棱/圆槽卵石牙)/L 舌尖(冰晶舌/嫩叶舌/紫光舌/朱砂舌)/FV 咬唇(叶片唇+白牙)全套手绘,躯干头盔换主题色斜面倒角,与 ember 同级(`coarseViseme` 降级通道删除)。**目的性游走**:闲逛不再是瞎转——约 1/3 的停留桶会挑一件正事:飞到「复习 N」徽章旁指点(有到期复习时)、打量下一个可学课程的球、回访你上次的卡点节点托腮(各保持 3s,确定性调度)。**情绪层**:连续答对 3 题(及每再 +3)触发 flame 得意反应(转圈+能量核金环脉动),答错清零。**记忆闭环动作**:打开笔记 tab=飞到画线旁掏本记录;卡点节点掌握毕业时(仪表盘卡点消失)自动打一个金色对勾。**桌宠模式(设置→伴学伙伴)**:应用外的透明置顶常驻窗——慢速巡游整个桌面工作区,指针进热区可戳可拖(离开热区自动恢复点击穿透),30s 轮询 XP 喂能量核、今日达标庆祝一次;开启时主窗内的伙伴隐身避免双影。**细节**:听写按住说话时胸屏跳动声纹波形条;栏锚点矩形查询 150ms 节流(省每帧强制布局)。
- **伴学 v10:连续性重设计 + 三个新交互 + R-06 肩甲** —— **闲时跨栏游走**:默认状态不再是"回左栏老家",而是在左中右栏间连续游走(时间桶调度,~30% 概率翻越到相邻栏;栖身左栏时仍走原生物理世界避球/被拍/天气,其他栏走面板空白带游弋),近大远小随实际栖身栏。**操作连续飞行**:所有移动改为**限速滑翔**(远距封顶巡航速度——治"闪现到某处";操作响应 0.85px/ms、跟句 0.6、闲逛 0.3),跨栏/操作切换都有起飞翻越动效;多操作并发(边朗读边输入)**跟随最新动作**(双槽信号:chat/notebook 各记激活时刻取最新,替代固定优先级);操作结束驻留片刻回 roam。**朗读句尾跟随(窄屏修复)**:锚点从"句子右侧(窄屏会出讲解区)"改为**高亮最后一行片段的右下角**——贴住最后一个字、不遮文字、全程钳在面板内(窄屏退让:句尾正下→行右→面板右下),手臂指向句尾。**记笔记新交互**:划线加笔记时飞到**那条画线旁**,掏出小本子和笔伏案记录(本笔弹入+笔尖写划动画,2.2s 钉住)。**打字新交互**:敲键盘=**敲自己的身体**(双臂内收 ±44° 交替敲胸腹,替代外拍),身体圆形屏幕**实时显示键入的字符**(打印字符随键击入状态,胸屏弹入定格;组合键/功能键不上屏)。**形象**:R-06 式**肩甲**(宽肩垫+受光棱+铆钉,共享骨骼全五形态披挂,各按主题色)。
- **伴学 v9:口型双路径 + 常驻 + 整句高亮** —— **口型重做(调研后拍板的混合路线)**:①**离线时间轴打底**(全引擎通用,含本地 kokoro/自定义云端/手机 Termux)——每句音频解码后做一次纯 JS FFT 帧分析(~几毫秒),生成 {时刻→嘴型} cue 表,播放时用音频时钟查表:采样级对齐,消灭实时分析器的平滑窗延迟与帧间抖动(这曾是"嘴型对不上读音"的主因);②**剧本口型加持**(默认 edge 档)——主进程开 TTS 引擎的逐词边界元数据(WordBoundary)+ 拼音声母映射(pinyin-pro,仅主进程),舌尖(L)/咬唇(FV)/齿擦(SS)/闭唇等**音频里听不出来的辅音嘴型**由剧本+引擎真值时序给出,随音频事件下发并缓存;两路共用 SpeechViseme 九形词表(Preston Blair 动画嘴型表子集),ember 形态新增 SS 齿擦(咧嘴露牙)/L 舌尖(舌顶上齿龈)/FV 咬唇(上牙咬下唇)全套手绘嘴型,其余形态自动降级最近母音。**伴学常驻**:无课程/空态也在(整个视口游走,不再随 worldReady 卸载),朗读中切课程不再消失。**朗读高亮整句对齐**:匹配改规范化(标点/全半角/引号差异归零)+ 句界扩展(句首吃开引号、句末吃标点闭引号),高亮不再落在句子一半的位置。附带修复:自定义 provider 模型条目缺必填 contextWindow 的类型错。
- **伴学 v8:入住感 + 渲染崩溃根治** —— **渲染失败修复(双层)**:①karaoke 高亮改用 **CSS Custom Highlight API**(`::highlight()` 注册 Range,零 DOM 改动)——旧 span 包裹改写 React 管理的 DOM,朗读逐句重渲染 reconcile 撞上外来节点即崩(重试重挂又好、下一句再炸);②`course:getNodeContent` 读取口对**原文与翻译都过净化**(围栏平衡/剥危险标签,幂等)——AI for beginners 这类含断行链接等畸形 markdown 的原文/翻译此前从未过净化直接喂渲染器。**右栏徘徊飞行**:无朗读时在讲解面板右侧空白带确定性游弋(住进文章里,避开顶部标签/按钮安全带),朗读时仍逐句跟读。**景深调档**:左栏加大到 76px/中栏 96px/右栏 120px(近大远小保留),手机切栏同样随实际栖身栏变大小。**左栏飞行**:天空无地面=无落地影,喷焰升级钢铁侠推进感(更长更亮+脉动辉光)。**按需求移除皇冠徽标**(光环保留)。
- **伴学 v7:近大远小 + 块面伪 3D + 起飞航灯 + 共振峰口型 + 手机端修复** —— **三栏=三重景深**:左栏远距离(60px)/中栏中距离(84px)/右栏近距离(108px),跨栏飞行时体型弹性过渡,近大远小的空间感;**形象块面伪 3D**:R-06 式倒角棱套件(左上受光棱+右下背光棱,硬边零渐变)应用到头盔/身体/耳舱,屏框用反向倒角做凹槽感,补通气缝/螺栓/裙甲 seam 等机械细节;**起飞动效**:跨栏起跳压缩蓄力→弹射 stretch,喷焰同步增强;**航灯**:左红右绿导航灯(航空惯例),常态慢呼吸、飞行时急闪交错;**口型升级为共振峰法**:F1/F2 双频段能量质心 → 声学元音三角最近邻判母音(响度驱动开口度),相邻元音双帧确认防抖;**修复手机端切换页面后 bot 消失**:家(左栏)被 T3 切栏卸载时退到当前在场的栏栖身,不再隐藏待刷新。
- **伴学 v6:朗读句级跟随(karaoke)** —— 整课朗读时讲解区**高亮正在读的那句话**(accent 蓝轻底,句子滚出视野才平滑居中跟随,不逐句乱跳),伴学伙伴**飞到当前句旁边抬起手臂指住这行字**,读一句挪一句(右侧站不下自动换到左侧、指向随侧翻转);朗读的**🔊 按钮固定悬浮在讲解视口右上角**,正文滚到哪都能一键停,不用滚回顶部。**中栏对话消息的朗读🔊同款**:正在念的消息逐句高亮,伴学留在中栏贴着句子指读(体型随栏切换)。句子定位与合成侧同源(`shared/speech-text` 纯函数切句),高亮跟随的是**播放序**而非合成到达序(合成快于收听,到达序会超前 1-2 句)。
- **伴学 v5:栏内临场感打磨** —— 中栏放大到 76px,**悬浮在输入框卡片上空**并在附近轻轻游动漂浮(慢利萨茹漂移,不是钉死)。**打字反馈升级**:从"手臂小抖"放大为**大幅拍臂(±52°,带回弹过冲)+ 整机随每次击键压弹一下**——一眼就知道他在跟你的键盘走。**右栏放大到 88px**:朗读讲解的口型动作看得更清楚。
- **伴学 v4:情境智能 + 可玩性 + 成长感** —— **情境反应**:进入考试节点飞来挥手加油(答题中零干扰红线不变)、课程导入时在左栏"监工"(豁免纱帘,成功=星星眼欢呼、失败=鼓励)、打开复习抽屉自豪待命、隔天回来星星眼转圈的加倍欢迎。**抓住他扔出去**:按住拖拽(挣扎抖动,触屏同款),快速甩出=真实物理翻滚晕眩、稳住后鼓脸生气,轻轻放下=开心。**宠物音效**:戳/被拍/落地/发送时的低音量合成啾哔声(零素材纯 WebAudio,设置可关)。**记忆联动**:他知道你昨天卡在哪——切课后先飞到薄弱节点的球旁托腮等你。**成长徽标**:XP 等级≥3 头顶小皇冠、≥7 金色光环,全形态共享。**世界细节**:飞行时喷焰粒子、地面阴影随高度缩放、左栏天风把他吹斜、雨天定期抖水、待机时落到最近的绳段/球顶上休息。
...
### Changed
- **伴学 v11.3:拆肩甲 + 跟读灵活避让** —— 按反馈**移除肩甲**(v10 的 R-06 式肩垫,五形态连同霜绒的肩部冰棱全拆,回归圆润轮廓);跟读锚点从固定右下角升级**多候选灵活避让**:优先落在句子末行右侧的空隙(站不下换首行左侧镜像、再退句尾正下方、面板角落),与高亮文字**零水平重叠**——bot 尽可能贴着正在读的那句、又不遮任何字。
- 内部:`npm run serve` 传任意参数(如 `--port`)时不再丢掉默认 `--data .serve-data` 一次性目录(曾静默落回 `~/.lookatstudy` 持久目录,测试数据污染)。
- **伴学伙伴 v3:一只生物,三个世界** —— 学习搭子从「三处各自守望」进化为**全应用唯一一只、跨栏连续行动的生命**:不再有分身,它在三重世界之间真实地飞来飞去(换栏时有飞行姿势与侧倾,像翻过栏边缘的墙)。**左栏是他的原生物理世界**:像钢铁侠一样悬浮飞行,遵循左栏的真实物理——巡航路过会撞开课程球,用户拖球甩他会被拍得翻滚晕眩(眼睛瞪圆,900ms 后稳住回正),锁定球是撞不动的墙。**中栏是他的宠物世界**:点进对话输入框,他飞来落在输入框上沿,像 Bongo Cat 一样逐键拍臂回应你的打字(位置避开文字)。**右栏是他的助教世界**:整课朗读时他飞到右栏对口型讲解;你划线加笔记,他会飞来做记笔记的动作。**纱帘待机**:你滚动屏幕或一段时间没有点击,他会退到半透明模糊的「纱帘后」安静待命(在左栏则收敛到天空静泊),点他一下或任何操作即刻唤醒回前台。三栏体型各不相同(左栏 88px 天空居民/中栏 76px 宠物/右栏 88px 助教),任何位置都不遮挡正文。五款形态选择不变,所有动效五形态共享。
...
### Fixed
- **伴学卡死左上角 + 朗读高亮句中断开(三层根因一起修)** —— ①Creature 的 rAF 循环 effect 依赖缺 `enabledLoaded`:首次运行时设置还没异步加载完、creature 未渲染(ref 为空)直接返回,之后 deps 不再变化循环永不启动——v9 移除 worldReady 门控时引入,所有状态下 bot 停在 DOM 默认左上角;②`shared/speech-text` 全角标点 typo(v0.x 起潜伏):HARD/SOFT 标点集里的全角 `！？；，：` 全写成了 ASCII 重复项——中文 `！？` 从不切句、全角逗号不算软标点,超长文本在任意位置硬断,朗读分块与高亮跟着在句子中间断开(旧测试用例全用 ASCII 标点,从未抓到);③**TTS 分块 ≠ 显示句**:超长强制断句(逗号处切,保合成流式)的块在渲染层用 `groupSentenceChunks` 并成完整显示句组,karaoke 高亮整组、组切换才重标。另:ui-test 新增伴学位置探针(空态在场+rail 家坐标),并修测试窗口 rAF 后台节流伪象(隐藏窗口暂停合成帧→坐标假冻结;探针判据=transform 已写入,恰好锁住 ① 的回归类)。
- **朗读音频句子乱序 + 高亮错位** —— ①**语音会把后面的句子插到前面念**(实测:课文最后一句插在第二句后面):渲染层解码队列按 `decodeAudioData` **完成序**进队(短句先解完),合成快于收听时全部到达一起竞争,播放顺序被打乱——此竞态自 v0.13 就存在,karaoke 让它显形。改为按 main 给的句序建**有序缓冲池**,只按序消费(下一句没到就等它,不跳句)。②**高亮标到没在念的文字**:句子匹配从"精确 indexOf+前缀兜底"升级为**token 序列+单调游标**——句子按空白切 token 在 DOM 文本里顺序找,token 间允许间隔(间隔=读与显示的合法差异:段落/列表边界、行内代码读时剥掉、表格竖线读成空格),句句推进游标不回跳(重复文本不再错标到前面)。③对话消息的匹配限定正文段落(思考块/工具产物渲染了文字但朗读不读,推理复述正文措辞极易误命中)。④讲解区与对话流两个朗读实例现在**互相通知停止**,切换朗读对象不再残留旧高亮。
- **自定义 provider 看图通道误判为 native 导致 400** —— custom provider 一律被当成能原生看图（`agent-engine.ts:245`），指向纯文本端点（如 GLM 主线）时文件直塞被拒，整轮对话崩掉。现改为：`custom_providers` 新增 `vision` 列，由用户在设置页显式声明"此模型支持看图"；`supportsVision` 未知模型从宽松 true 改保守 false（猜错的代价从整轮 400 降为桥接/不喂）；另加 400 保险丝——首 part 即错且命中拒图措辞时自动降级重试（配了视觉覆盖走桥接、无覆盖跳过图片），附可见提示。**触发条件**：多模态导入 flag 开 + hook 猜测消息含"图"字 + 主模型为 custom 纯文本 provider（如 dev 环境 `.env` 种子）。
