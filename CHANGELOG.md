...
### Added
- **伴学细化:待机空地 + 表情口型动作打磨** —— 左栏待机点从顶部天空挪到**高度中部空域**,并自动**挑选离所有课程球最远的空地**悬浮(每 3 秒或滚动换景后重选,球挪走他跟着挪窝;避让场兜底过渡)。**动作反馈**:戳他的反应花样轮换(跳跳→挥手→转圈),窗口切回来从"跳一下"升级为**挥手打招呼**;跨栏飞行落栖输入框/右栏时有一个**落地压缩回弹**。**口型**:同一开口度下按母音横向收放(展唇 E/I 横宽、圆唇 O/U 收窄),口型从"只有高度"升级为"高×宽"两维,说话更生动。**表情细节**:星星眼高频闪烁,打盹时整机睡眠呼吸起伏。reduced-motion 下全部退静态。

### Changed
- **伴学伙伴 v3:一只生物,三个世界** —— 学习搭子从「三处各自守望」进化为**全应用唯一一只、跨栏连续行动的生命**:不再有分身,它在三重世界之间真实地飞来飞去(换栏时有飞行姿势与侧倾,像翻过栏边缘的墙)。**左栏是他的原生物理世界**:像钢铁侠一样悬浮飞行,遵循左栏的真实物理——巡航路过会撞开课程球,用户拖球甩他会被拍得翻滚晕眩(眼睛瞪圆,900ms 后稳住回正),锁定球是撞不动的墙。**中栏是他的宠物世界**:点进对话输入框,他飞来落在输入框上沿,像 Bongo Cat 一样逐键拍臂回应你的打字(位置避开文字)。**右栏是他的助教世界**:整课朗读时他飞到右栏对口型讲解;你划线加笔记,他会飞来做记笔记的动作。**纱帘待机**:你滚动屏幕或一段时间没有点击,他会退到半透明模糊的「纱帘后」安静待命(在左栏则收敛到天空静泊),点他一下或任何操作即刻唤醒回前台。三栏体型各不相同(左栏 88px 天空居民/中栏 56px 宠物/右栏 64px 助教),任何位置都不遮挡正文。五款形态选择不变,所有动效五形态共享。
- **修复:整课朗读口型自 v1 起实际未生效** —— 朗读状态信号此前在组件层按节点 id 对比转发,存在静默失效路径(朗读在放、信号没发)。现已下沉到语音引擎层按播放事实直发,朗读口型同步从此真正可用(伴学助教讲解随之上线)。

### Changed
- **伴学自主巡航避让球体** —— 左栏世界里生物巡航路过课程球时不再直愣愣撞过去搅乱球链布局:新增避让转向场,球进入感知范围即产生推离转向(贴得越近推得越强,正在冲向球时按接近速度加倍制动),近距避让优先于巡航目的——他宁可绕路也不穿球。被你拖球拍他的玩法不受影响(那是晕眩路径,不做避让)。仿真断言锁进 verify:自主行动全程与球保持分离、绕行平滑无瞬移。
- **伴学架构:组件→伴学总线的触发全部改走 window 事件** —— 打包器在 Windows 双盘 junction 环境下可能把总线模块解析成两个实例(组件直调写进副本、生物订阅正主,事件全丢)。所有触发(聚焦/朗读/听写/发送/记笔记/拍击/麦克风包络/左栏世界注册)统一 CustomEvent 广播,天然单例,对任何打包路径免疫(与设置开关事件同款模式)。
- **伴学伙伴 v2:五款可切换形象** —— 同一套 cel 描线设计语言下的五个性格形态,设置页「伴学伙伴」区新增形象选择器(五张卡片,实时预览,点选即换、全应用三处栖息地同步换装):**小焰**(Ember,珊瑚装甲+青屏,家族原点)、**霜绒**(Frost,六瓣冰晶能量核+棱面头盔,冷静可靠)、**苔芽**(Moss,双子叶随 XP 绽开+叶耳,安静生长)、**星尘**(Astro,暗夜观景窗+月相能量核反转配色,星野浪漫)、**墨墨**(Ink,宣纸身+朱砂印章+毛笔天线,书卷气)。每个形态有自己的一套眼型/口型/耳朵/天线与 streak 签名(连击时:金焰/晶簇闪光/开花/彗尾/朱砂笔尖),能量核形态各异但都如实反映今日 XP;打字、点击、眨眼、视线、声波等全部动效五形态零差别生效(壳+皮肤架构,生命系统只有一份)。
- **伴学伙伴 v2:Bongo Cat 式输入反馈** —— 学习搭子现在看得见你的双手:打字时逐键交替拍小手臂(左右左,跟着你的节奏起落,停 1.2 秒放下);鼠标点击哪半屏就朝哪边拍臂;语音听写时胸口声波弧随**真实麦克风音量**四档起伏(快起慢落,像真的在听);发出消息时一个开心小出拳;窗口切走 4 秒就地打盹,切回来跳一下打招呼。另新增**中栏对话栖息地**(对话流右上角小形态,T3 窄屏单栏视图也常驻),至此左中右三栏都有居民。全部为渲染层实现,零 IPC 协议改动。
- **伴学伙伴「小焰」(Ember)** —— R-06×R-03 cel 风格的 SVG 悬浮守护机,常驻学习全程:左栏课程地图的天空里守望,选中节点后右栏笔记本右下角以导师/桌宠形态在场;点它一下会打招呼(问候轮换)。**全动作反馈**:答对冲拳庆祝、答错摊手鼓励(绝不嘲讽)、拿皇冠加冕金冠、考试通过星星眼、连击点燃天线火苗、能量核随今日 XP 充能、AI 流式回答时托腮思考、语音模式侧耳倾听(带声波)、空闲 3 分钟打盹 zzz。**朗读口型同步**:整课朗读时从真实音频实时取响度+频谱质心,口型在闭/A/E/I/O/U 六档母音形状间切换(不是单纯张嘴)——AI 软件式口型;分析节点直通朗读链,零协议改动、异常自动退回直连。设置页新增「伴学伙伴」开关(默认开,关闭=栖息地完全不渲染)。纯渲染层实现,不碰 DB 与 IPC 协议;reduced-motion 自动退静态;点击只落在角色涂色区,不挡地图球与正文选区。

### Fixed
- **伴学左栏世界"瞬移"** —— 悬浮控制器的力单位换算错了两处:Matter 的力换算是 Δv=力/质量×dt²(≈278 倍),控制器加速度没除 dt²(弹簧等效放大 278 倍→速度 40+px/步钉墙抖动);重力补偿符号也写反(y 正方向向下,补偿须为负,写正=重力加倍沉底)。修正后生物在巡航点平稳悬浮(实测每帧位移 2px),穿越球链不再连跳;速度/多球推挤均加合成钳制,并把"连续性+真悬浮+穿球链"三项稳定性仿真锁进 verify(防单位回归)。
- **导入面板标签栏文字重叠** —— v0.16 把导入入口加到六个(网址/MD/文件夹/电子书/音频/课程包)后,左栏 300px 宽度里六个不换行标签挤在一行互相叠字。改为弹性换行:窄栏自动排成两行(3+3),拉宽(T3 地图全宽)自动回到一行,任何宽度都不再重叠。
- **手机端设置页自定义模型摘要溢出屏幕** —— 视觉/语音模型区"已配置自定义"摘要行里的接口地址(baseURL)整段不换行,手机窄屏上把面板横向撑出屏幕。地址改为随宽度自动折行(与旁边的模型名同样处理),任何长度的 URL 都不再溢出。

## [0.16.0] - 2026-08-20

### Added
- **视频链接导入(B站直连 / YouTube 等经 yt-dlp)** —— 智能 URL 框现在认视频链接:`bilibili.com`(BV/av,含 b23.tv 短链)免登录直连拉 DASH 音轨(纯 JS 实现 B站 wbi 签名);YouTube/抖音等上千站点检测本机已装的 yt-dlp,**字幕优先**——有 CC/自动字幕直接出文本,零转写零模型,无字幕才下载音轨走 Whisper 转写(与音频导入同款,模型缺了自动下载、可取消);未装 yt-dlp 给出平台安装指引。视频内容照常进 AI 结构设计成课,音轨字节随快照缓存,断点续跑零二次下载。
- **B站多分P 整季导入** —— 多分P视频(课程/番剧/合集)不带 `?p=` 导入即整季:每P一段音轨逐段转写,各成虚拟文档,AI 按集分章(同多集播客);带 `?p=N` 只导该集,两者是不同课程。整季有 200P 封顶(病态合集保护,进度里明示);同一视频再次导入直接从快照恢复,零拉流零转写。
- **本地视频文件导入** —— 音频入口升级为「音频/视频」:mp4/m4v/mov 直接取音轨转写(B站等 DASH 源的 fragmented MP4 由内置 fMP4→ADTS 转封装支持,零外部工具;mkv/webm 提示先转 mp4)。
- **docx 文档导入** —— 文件夹导入支持 .docx(Word):officeparser 解析,标题样式(Heading 1/2…)保真为 markdown 标题,管线自动按标题拆课。零新依赖。
- **课程包导出开放到全部新来源** —— 网页文章/arXiv、粘贴文本、EPUB 电子书、音频/视频转写的课程现在都能导出课程包:正文随快照携带(docCache),接收方导入零网络、零 AI、零转写。本地文件夹课程仍不可导出(含本机私有路径);github 包沿用 owner-repo 命名,新源用课程标题命名。

- **智能链接导入** —— 导入面板的 URL 框升级为一个框装三类链接:`github.com` 仓库走完整导入,`arxiv.org`(abs/pdf,新式与旧式 ID)自动下载论文 PDF 提取文本,其余任意 http(s) 链接抽取网页正文成课(纯 JS 的 readability 正文抽取,剥导航/页脚噪声,图片地址自动绝对化;非文章页诚实报错而不是把整页噪声灌进课程)。
- **EPUB 电子书导入** —— 新「电子书」入口:每章一课,章节边界来自书的书脊顺序与目录(EPUB3 nav / EPUB2 ncx 都认),封面页自动跳过。书的**内容哈希**做身份——重新打包不换课程,正文真的改了才重走 AI。文件夹导入也认 `.epub`(整本压平,章标题降级为小节标题照样拆课)。
- **粘贴长文升级到完整导入管线** —— 粘贴 tab 从旧的同步"生成课程"切到与仓库导入同级的后台任务:无小标题的超长文本按句子边界自动分段成多课,获得 AI 结构设计、进度显示与断点续跑;课程名称改为可选。旧通道保留兼容。
...
- **本地音频导入(播客/讲座转写成课)** —— 新「音频」入口:wav/mp3/m4a/flac(及 aac/ogg/opus)本地解码后用 Whisper 分段转写(60 秒一段,进度滚动、可取消;段间释放解码队列,不阻塞正在进行的听写),转写稿自动分段成课;多文件=多集播客,按集成章。缺 Whisper 模型时自动下载(约 1GB);Termux 等无本地引擎的平台诚实报错。音频字节哈希做课程身份——同一批文件再导直接复用,零重复转写。
...
- **虚拟文档源(url/粘贴/电子书)的断点续跑与漂移检测** —— 正文随导入快照落盘(`docCache`),链接失效或原文件移动后仍可从断点继续;漂移检测从"路径集合哈希"升级为"内容哈希"(这类源改内容不改路径,路径哈希会漏检漂移静默复用旧结构)。

### Fixed
- **抖音链接路由修复** —— 抖音视频链接(douyin.com / v.douyin.com 短链 / iesdouyin.com 分享链)此前没进视频路由表,会掉进"网页文章"路径抽取失败;现在正确分流到 yt-dlp 视频导入(需自装 yt-dlp,与 YouTube 同款)。
- **arXiv 论文标题抓取修复** —— abs 页标题解析的贪婪正则回溯后只捕获到最后一个字符(真实事故:《Attention Is All You Need》标题抓成 "d")。改为 `citation_title` meta 优先(arXiv 页必带,最可靠)+ `<title>` 剥 `[论文号]` 前缀兜底的纯函数 `extractArxivTitle`,带真实页面形状的回归测试。
- **导入来源 live 核查套件** —— 新增 `live-test-import-sources.mjs`:arXiv 真论文、真实网页正文、Gutenberg 真书 EPUB、python-docx 生成真 Word 文件、Whisper 真实转写(60 秒 B站音轨)、yt-dlp 真实拉起(未装/模型缺/python 缺时对应项 SKIP),六项全绿——非仓库来源的获取与解析层从此有可重复的真实链路验证。
- **设置页模型区两处交互修正(v0.15 验收反馈)** —— ①语音的「自定义」按钮点击后不亮(激活态此前只看已保存引擎,点了没保存无反馈):现在点击即进入激活态展开配置,已有自定义 provider 在用时保持常亮;切回 Edge 在线/本地离线自动收起自定义配置块。②看图模型区去掉「复用主模型 / 自定义」切换按钮——标题本就写着"留空则复用主模型",两按钮是多余决策:自定义填写窗口直接常显(已配覆盖时显示摘要 + 测试/更换/停止,未配置时显示已有自定义一键选用 + 新建表单),无覆盖即天然复用主模型。
...
- **Termux 语音引擎改为默认安装,安装全程零交互** —— 引擎包约 12MB,相对 Node 与系统升级的体量可忽略,不再可选/不再中途提问(y/N 一并移除);失败不阻断安装(朗读在线档/听写云档不依赖本地引擎),重跑安装命令即补装。`--voice` 参数仍被接受(传不传一样)。

### Removed
- **Azure 朗读 / Azure 听写 / Groq 听写的预设入口**(设置页不再提供;老库已配置的用户后端照常生效,界面显示"旧配置,仍生效"):云端语音统一由自定义 provider 承接(OpenAI 兼容端点)。

### Fixed
- 两处语音误导文案:安装器内的语音模型大小提示还是旧引擎时代数字(听写 200MB → 实为云档建议/本地 360MB~1GB);界面"引擎未就位"提示教用户跑不落盘的 `bash install-termux.sh --voice`。
