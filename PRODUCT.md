...
3. **AI agent panel** (chat, middle, `surface-1`, minimal-reading-flow / claude.ai style) — content is the only protagonist, toolbars recede. Streaming chat with parts-based rendering (text / reasoning-foldable / tool-call with lucide Wrench/XCircle / proposal-apply). User messages: right-aligned faint tint, no bubble. AI messages: left-aligned, no bubble. Markdown with GFM, 80ch prose cap. Starter prompts as faint pills above input. **Soul pills inside the input box** (teaching persona 风格: 精讲/引导/实战, collapsible, `null` = off), each with lucide icon + tooltip explaining when to use. Cmd+K command palette.
...
7. **Settings drawer** (overlay from header gear, `surface-0`) — BYOK provider config, custom providers, AI-vision toggle, appearance & language.
