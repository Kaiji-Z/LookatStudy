...
"# 欢迎使用 LookatStudy",
"",
"**LookatStudy** 是一个本地优先、AI 驱动的桌面学习平台。它能把学习仓库、本地文件夹、网页链接、论文、视频或电子书变成一个多邻国式的互动课程——带有技能地图、AI 导师、间隔重复和游戏化激励。",
"",
"## 它能帮你做什么",
"",
"- 📚 **把几乎任何东西变课程**：GitHub 仓库、本地文件夹、网页文章、arXiv 论文、B站/YouTube 视频、EPUB 电子书、粘贴的文本、本地录音录像——12 种文档格式（Markdown / Jupyter / RST / PDF / PPTX / Word / EPUB 等）+ 30 多种代码语言自动解析成结构化课程，图片也能带进来",
...
"",
"**LookatStudy** is a local-first, AI-driven desktop learning platform. It turns a repo, a local folder, a web article, a paper, a video, or an e-book into a Duolingo-style interactive course — complete with a skill map, an AI tutor, spaced repetition, and playful incentives.",
"",
"## What it can do for you",
"",
"- 📚 **Turn almost anything into a course**: a GitHub repo, a local folder, a web article, an arXiv paper, a Bilibili/YouTube video, an EPUB book, pasted text, or local recordings — 12 document formats (Markdown / Jupyter / RST / PDF / PPTX / Word / EPUB and more) plus 30+ code languages are parsed into a structured course, images included",
...
"LookatStudy always opens in the “no course selected” state: pick an imported course in the “Import courses” panel on the left, or import something new. Open the next lesson, “Import Your First Course”, for all the ways in!",
      ...
      },
      {
        ...
        summary: "五种导入方式、后台导入任务、课程删除",
        summaryEn: "Five import routes, background import jobs, and course removal",
        content: m(
"# 导入你的第一个课程",
"",
"LookatStudy 支持五种导入方式，覆盖几乎所有学习材料来源。",
"",
"## 方式一：GitHub URL",
"",
...
"## 方式二：本地文件夹",
"",
"点击本地文件夹按钮，选择一个包含学习材料的文件夹。支持递归扫描子目录，自动识别文档、代码、图片、翻译。文件夹里的音频（.mp3/.m4a 等）和视频（.mp4 等）文件会在本地 Whisper 转写成课文，一节录音变成一节课。",
"",
"适用场景：下载的课程包、本地 clone 的仓库、自己整理的学习笔记文件夹。",
"",
"## 方式三：智能链接",
"",
"直接粘贴一个网页地址。文章页抽取正文成课；arXiv 论文下载 PDF 解析；视频链接（B站/YouTube/抖音）拉取音频转写，有字幕的视频优先用字幕、零转写——B站多分P整季导入，一集一课。",
"",
"## 方式四：粘贴 Markdown",
"",
...
"## 方式五：EPUB 电子书",
"",
"选择一个 .epub 文件，每章自动变成一节课，章节结构完整保留。",
"",
"## 导入是后台任务",
"",
...
"",
"LookatStudy supports five import paths that cover nearly every source of learning material.",
"",
"## Path 1: GitHub URL",
"",
...
"## Path 2: Local folder",
"",
"Click the local-folder button and pick a folder of learning material. Subdirectories are scanned recursively; documents, code, images and translations are recognized automatically. Audio files (.mp3, .m4a, and friends) and videos (.mp4, ...) inside the folder are transcribed into lessons by local Whisper — one recording becomes one lesson.",
"",
"Good for: downloaded course bundles, locally cloned repos, or a folder of notes you curated yourself.",
"",
"## Path 3: Smart links",
"",
"Paste any web address. Articles are extracted into lessons, arXiv papers download and parse their PDFs, and video links (Bilibili / YouTube / Douyin) pull the audio for transcription — subtitled videos prefer subtitles, zero transcription. A multi-part Bilibili series imports as a whole season, one part per lesson.",
"",
"## Path 4: Pasted Markdown",
"",
...
"## Path 5: EPUB books",
"",
"Pick an .epub file; each chapter becomes a lesson, with the book structure preserved.",
"",
...
"## 异步会话",
"",
"AI 正在回复时也可以随意切换课时和会话。原对话在后台继续输出，会话标签和地图球上会转圈提示；切回来时，回答原样显示。同一段会话在一个回答结束前只接受一条提问，跨会话可以并行提问。",
        ),
        contentEn: m(
...
"## Async sessions",
"",
"While the AI is replying you are free to switch lessons or threads. The conversation keeps streaming in the background, with a spinner on its tab and on its map node, and the answer is right there when you come back. Within one thread only one send runs at a time; across threads you can ask in parallel.",
        ),
      },
    ],
  },
...
"## 每题限时",
"",
"每道题独立倒计时，时长按题目本身估算——题干越长、选项越多、含代码或公式，时间越多，下限 60 秒、上限 5 分钟。到时自动记录当前选择并进入下一题。",
"",
"## 离开即终止",
"",
...
"",
"Each question runs its own countdown, sized to the question itself — a longer stem, more options, code, or formulas each buy more time, clamped between 60 seconds and five minutes. When time runs out, the current choice is recorded and the exam moves on.",
"",
...
"## 三个 Tab",
"",
"右栏顶部有三个 Tab：讲解 Tab 显示课时正文（Markdown 渲染，可切脑图总览），笔记 Tab 展示上述三区，黑板 Tab 是对话中最新重产物（概念图/流程图/对比表/代码讲解）的大画布，可缩放平移，AI 生成新图时自动翻过去。讲解和笔记分开，让你在阅读正文时不被笔记干扰。",
        ),
        contentEn: m(
...
"## Three tabs",
"",
"The top of the right pane has three tabs: the Lesson tab shows the lesson body (rendered Markdown, with a mind-map overview toggle); the Notes tab shows the three zones above; and the Board tab is a zoomable canvas for the latest heavy artifact from the chat (concept map, flow diagram, compare table, or code walkthrough) — it flips over by itself when the AI produces a new one. Keeping lesson and notes separate lets you read without clutter.",
        ),
      },
      {
        title: "画线加笔记与溯源跳转",
...
"## 生成式 UI 存在哪",
"",
"所有 AI 生成的组件出现在中栏对话流和右栏笔记 Tab 的理解区（持久保存），四类重产物还会登上黑板 Tab 的大画布。AI 生成的概念图不是聊完就消失——它们留在你的笔记本里随时回顾。",
        ),
        contentEn: m(
"# Generative UI",
"",
...
"## Where they live",
"",
"Generated components appear in the middle-pane chat stream and in the Understand zone of the Notes tab (persisted), and the four heavy artifacts also land on the Board canvas. An AI concept map does not vanish when the chat ends — it stays in your notebook for review.",
        ),
      },
    ],
  },
...
"顺便一提：你正在读的这篇使用指南本身就是双语的——上文说的 🌐 切换器就在它的地图标题卡上，可以直接试。",
        ),
        contentEn: m(
