...
- **网页来源的正文边界(2026-08-23)**:网页抽取的文件(url/arXiv/文章类)尾部常残留推广段、关注引导、
  相关阅读等非正文内容。判断"尾部一段是否属于正文"是语义判断,交给你:明显是作者行文一部分的保留;
  明显是模板或与主题无关的推广小段,不要为它单独建 lesson,anchor 也不要截到它(正文在哪结束你说了算)
- **书籍的前置/外围小页(2026-08-23)**:电子书来源可能有出版社宣传页、作者/译者简介、Digital Lab 之类
  的出版方介绍页——标题叫"未命名章节"的文件是解析层诚实标注"无标题"(不是原书标题),按内容主题命名。
  明显与书的内容无关的外围小页(出版方介绍/宣传语),不要为它单独建 lesson,并入首章或忽略
