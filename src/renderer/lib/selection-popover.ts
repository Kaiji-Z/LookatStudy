...
export function selectionPopoverPosition(
  ...
  const GAP = 8;
  const midY = sel.top + sel.height / 2;
  if (sel.right + GAP + popoverW <= containerWidth) {
    return { left: sel.right + GAP, top: midY, transform: "translate(0, -50%)" };
  }
  if (sel.left - GAP - popoverW >= 0) {
    return { left: sel.left - GAP, top: midY, transform: "translate(-100%, -50%)" };
  }
  return { left: sel.left, top: sel.bottom + GAP, transform: "translate(0, 0)" };
}
